...
public class GraphQuery implements QueryDispatcher {
	...
	public PlasmaDataGraph[] find(Query query, int requestMax,
	...
	private int countResults(Query query, PlasmaType type) {
		...
		// columns whatsoever.
		PlasmaDataGraph[] graphs = find(query, type,
				new Timestamp(System.currentTimeMillis()));
		return graphs.length;
	}

	private PlasmaDataGraph[] findResults(Query query, PlasmaType type,
			Timestamp snapshotDate) {
		return find(query, type, snapshotDate);
	}

	private PlasmaDataGraph[] find(Query query, PlasmaType type,
			Timestamp snapshotDate) {
		...
		// in the Select.
		HBaseFilterAssembler columnFilterAssembler = createRootColumnFilterAssembler(
				type, selectionCollector);
		...
		HBaseGraphAssembler graphAssembler = createGraphAssembler(query, type,
				graphReader, selectionCollector, snapshotDate);
		...
		if (where == null
				|| (partialScans.size() == 0 && fuzzyScans.size() == 0 && completeKeys
						.size() == 0)) {
		...
		ResultsAssembler resultsCollector = new SlidingResultsAssembler(
				graphRecognizerRootExpr, orderingComparator, rootTableReader,
				graphAssembler, query.getStartRange(), query.getEndRange());
		...
		try {
			...
			if (partialScans.size() > 0 || fuzzyScans.size() > 0
				...
				for (CompleteRowKey key : completeKeys) {
					if (resultsCollector.isResultEndRangeReached())
						break;
					execute(key, rootTableReader, columnFilter,
							resultsCollector);
				} // scan
				for (PartialRowKey scan : partialScans) {
					if (resultsCollector.isResultEndRangeReached())
						break;
					execute(scan, rootTableReader, columnFilter,
							resultsCollector);
				} // scan
			...
			if (log.isDebugEnabled()) {
				...
				log.debug("return "
						+ String.valueOf(resultsCollector.size())
						+ " assembled, "
						+ String.valueOf(resultsCollector.getIgnoredResults())
						+ " ignored, "
						+ String.valueOf(resultsCollector
								.getUnrecognizedResults()) + " unrecognized ("
	...
	private boolean canAbortScan(boolean hasOrdering, Integer startRange,
			Integer endRange, Set<PlasmaDataGraph> result) {
	...
	private void execute(PartialRowKey partialRowKey,
			TableReader rootTableReader, Filter columnFilter,
	...
	private void execute(CompleteRowKey rowKey, TableReader rootTableReader,
		...
		if (log.isDebugEnabled())
			log.debug("using row key get: (" + "row: '"
					+ Bytes.toString(get.getRow()) + "'");
		execute(get, rootTableReader, collector);
	}

	private void execute(FuzzyRowKey fuzzyScan, TableReader rootTableReader,
			Filter columnFilter, ResultsAssembler collector) throws IOException {
		Scan scan = createScan(fuzzyScan, columnFilter);
		execute(scan, rootTableReader, collector);
	}

	private Scan createScan(FuzzyRowKey fuzzyScan, Filter columnFilter)
			throws IOException {
		FilterList rootFilter = new FilterList(
				FilterList.Operator.MUST_PASS_ALL);
		rootFilter.addFilter(columnFilter);
	...
	private void execute(Get get, TableReader rootTableReader,
			ResultsAssembler collector) throws IOException {
	...
	private void execute(Scan scan, TableReader rootTableReader,
			ResultsAssembler collector) throws IOException {
	 ...
	 * existence of path predicates in the Select. If no path predicates exist
	 * in the selection, we know the entire selection graph can be fetched in
	 * one round trip, so {@link GraphFetchColumnFilterAssembler} is create.
	 ...
	 */
	private HBaseFilterAssembler createRootColumnFilterAssembler(
			PlasmaType type, SelectionCollector collector) {
		HBaseFilterAssembler columnFilterAssembler = null;
		if (collector.getPredicateMap().size() > 0) {
			columnFilterAssembler = new InitialFetchColumnFilterAssembler(
					collector, type);
		} else {
			columnFilterAssembler = new GraphFetchColumnFilterAssembler(
					collector, type);
		}
	...
	/**
	 * Recursively collects row key properties adding them to the given
	 * collector for the given type. Any additional properties associated with
	 * types discovered during traversal of user defined row key field paths are
	 * added recursively.
	 * 
	 * @param collector
	 ...
	 *            the current type
	 */
	private void collectRowKeyProperties(SelectionCollector collector,
			PlasmaType type) {
	...
	/**
	 * Create a specific graph assembler based on the existence of selection
	 * path predicates found in the given collector. Since the reader hierarchy
	 * is initialized based entirely on metadata found in the selection graph,
	 ...
	 */
	private HBaseGraphAssembler createGraphAssembler(Query query,
			PlasmaType type, DistributedReader graphReader,
			Selection selection, Timestamp snapshotDate) {
		HBaseGraphAssembler graphAssembler = null;

		QueryFetchType fetchType = CloudGraphConfigProp
				.getQueryFetchType(query);
		switch (fetchType) {
			case PARALLEL :
				...
				if (minPool > maxPool)
					minPool = maxPool;
				int threadMaxDepth = CloudGraphConfigProp
						.getQueryThreadMaxDepth(query);
				ConfigProps config = new ConfigProps(minPool, maxPool,
						threadMaxDepth);
				if (selection.hasPredicates()) {
					graphAssembler = new ParallelGraphSliceAssembler(type,
							selection, graphReader, snapshotDate, config);
				} else {
			...
			default :
				if (selection.hasPredicates()) {
					graphAssembler = new GraphSliceAssembler(type, selection,
							graphReader, snapshotDate);
				} else {
					graphAssembler = new GraphAssembler(type, selection,
							graphReader, snapshotDate);
				}
				break;
		}

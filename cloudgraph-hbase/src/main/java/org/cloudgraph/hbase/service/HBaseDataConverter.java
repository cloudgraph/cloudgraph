...
public class HBaseDataConverter {
	 ...
	 * information for the given property. For 'many' or multi-valued
	 * properties, if the SDO Java type for the property is not already String,
	 * the value is first converted to String using the SDO conversion which
	 * uses java.util.Arrays formatting.
	 * 
	 ...
	 * information for the given property. For 'many' or multi-valued
	 * properties, if the SDO Java type for the property is not already String,
	 * the value is first converted from a String using the SDO conversion which
	 * uses java.util.Arrays formatting, resulting in an array of primitive
	 * types. For non 'many' or singular properties, only the HBase Bytes
	 * utility is used.
	 * 
	 ...
	 * properties, if the SDO Java type for the property is not already String,
	 * the value is first converted to String using the SDO conversion which
	 * uses java.util.Arrays formatting. For non 'many' or singular properties,
	 * only the HBase Bytes utility is used.
	 * 
	 * @param sourceProperty
	 *            the source property
	 * @param value
	...
	public byte[] toBytes(Property sourceProperty, Object value) {
		...
		DataType dataType = DataType
				.valueOf(sourceProperty.getType().getName());
		...
		// Data types stored as String bytes in HBase
			...
			case Bytes :
				if (!sourceProperty.isMany()) {
					byte[] resultBytes = DataConverter.INSTANCE.toBytes(
			...
			case Boolean :
				if (!sourceProperty.isMany()) {
					boolean resultBool = DataConverter.INSTANCE.toBoolean(
							sourceProperty.getType(), value);
			...
			case Double :
				if (!sourceProperty.isMany()) {
					double resultDouble = DataConverter.INSTANCE.toDouble(
							sourceProperty.getType(), value);
			...
			case Float :
				if (!sourceProperty.isMany()) {
					float resultFloat = DataConverter.INSTANCE.toFloat(
							sourceProperty.getType(), value);
			...
			case Int :
				if (!sourceProperty.isMany()) {
					int resultInt = DataConverter.INSTANCE.toInt(
							sourceProperty.getType(), value);
			...
			case Short :
				if (!sourceProperty.isMany()) {
					short resultShort = DataConverter.INSTANCE.toShort(
							sourceProperty.getType(), value);

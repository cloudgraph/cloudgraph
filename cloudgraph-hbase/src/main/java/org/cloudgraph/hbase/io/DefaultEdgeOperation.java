...
public abstract class DefaultEdgeOperation implements EdgeOperation {
	...
	protected DefaultEdgeOperation(
			PlasmaType sourceType,
     ...
     * If the opposite type is not bound (to a table), the type is stored in the
	 * local row and we write its sequences for this edge. Or if this type is
	 * not bound to a table, we also know its opposite edges must be local
	 * as well. 
...
//	}
	...
	//FIXME: tab character therefore cannot be found in any field of any row key
	// otherwise maybe use protobuf
	...
	protected void decodeRowKeys(byte[] rowKeys) {
		String[] tokens = (new String(rowKeys, charset)).split("[\\t]");
		this.rowKeys = new ArrayList<String>(tokens.length);
...
//			byte[] subtypeBytes = org.cloudgraph.common.Bytes.concat(String.valueOf(seq).getBytes(charset), 
//					TYPE_DELIM.getBytes(charset), typeBytes);	
//			buf.put(subtypeBytes);
//			i++;
//		}
//	    byte[] result = new byte[buf.position()];
//	    System.arraycopy(buf.array(), buf.arrayOffset(), result, 0, buf.position()); 
//		return result;
//	}
//	
...
//			PlasmaType type = (PlasmaType)PlasmaTypeHelper.INSTANCE.getType(tokens[1], tokens[2]);
...
//	}
	...
	protected void decodeDefaultSubType(byte[] typeBytes)
	{
	    ...
	    this.collectionDefaultSubType = (PlasmaType)PlasmaTypeHelper.INSTANCE.getType(
	    		tokens[0], tokens[1]);
	}
	...
	{
	    String[] tokens = (new String(typeBytes, this.charset)).split(this.columnKeyFeildDelimRegexp);
	    this.collectionBaseType = (PlasmaType)PlasmaTypeHelper.INSTANCE.getType(
	    		tokens[0], tokens[1]);
	}

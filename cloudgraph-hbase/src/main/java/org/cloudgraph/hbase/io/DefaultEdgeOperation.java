...
public abstract class DefaultEdgeOperation implements EdgeOperation {
	...
	protected DefaultEdgeOperation(PlasmaType sourceType,
			PlasmaProperty sourceProp, TableConfig tableConfig,
		...
		this.columnKeyFeildDelim = graphConfig
				.getColumnKeyFieldDelimiterBytes();
		this.columnKeyFeildDelimRegexp = graphConfig
				.getColumnKeyFieldDelimiter();
	...
	/**
	 * Calculates and returns whether the edges are local. If the opposite type
	 * is not bound (to a table), the type is stored in the local row and we
	 * write its sequences for this edge. Or if this type is not bound to a
	 * table, we also know its opposite edges must be local as well.
	 * 
	...
	public String getTable() {
		if (!isExternal())
			throw new IllegalStateException(
					"cannot find table for local (non-external) edge, " + this);
	...
	protected void decodeDefaultSubType(byte[] typeBytes) {
		String[] tokens = (new String(typeBytes, this.charset))
				.split(this.columnKeyFeildDelimRegexp);
		this.collectionDefaultSubType = (PlasmaType) PlasmaTypeHelper.INSTANCE
				.getType(tokens[0], tokens[1]);
	}
	...
	protected void decodeCollectionBaseType(byte[] typeBytes) {
		String[] tokens = (new String(typeBytes, this.charset))
				.split(this.columnKeyFeildDelimRegexp);
		this.collectionBaseType = (PlasmaType) PlasmaTypeHelper.INSTANCE
				.getType(tokens[0], tokens[1]);
	}
	...
	private byte[] encodeType(PlasmaType type) {
		...
		if (uriPhysicalName != null && physicalName != null) {
			return org.cloudgraph.common.Bytes.concat(uriPhysicalName,
					TYPE_DELIM.getBytes(charset), physicalName);
		} else {
			...
			return org.cloudgraph.common.Bytes.concat(uri,
					TYPE_DELIM.getBytes(charset), name);
		}
	}

	public static PlasmaType decodeType(byte[] typeBytes) {
		String[] tokens = (new String(typeBytes, charset))
				.split(TYPE_DELIM_REGEXP);
		PlasmaType result = (PlasmaType) PlasmaTypeHelper.INSTANCE.getType(
				tokens[0], tokens[1]);
		return result;
	}
	...
	protected byte[] fetchValue(byte[] family, byte[] qualifier, ColumnMap row) {
		if (!row.containsColumn(family, qualifier)) {
			throw new RequiredKeyFieldException("expected column qualifier '"
					+ new String(qualifier) + "' for reference property, "
					+ sourceProp);
		}

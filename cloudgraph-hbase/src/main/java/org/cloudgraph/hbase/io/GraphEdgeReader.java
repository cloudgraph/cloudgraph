...
public class GraphEdgeReader extends DefaultEdgeOperation implements EdgeReader {
	...
	GraphEdgeReader(PlasmaType sourceType, PlasmaProperty sourceProp,
			Long typeSequenceNum, TableConfig tableConfig,
			DataGraphConfig graphConfig, RowReader rowReader)
			throws IOException {
		...
		this.baseTypeQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, typeSequenceNum, sourceProp,
				EdgeMetaKey.BASETYPE);
		this.pathQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, typeSequenceNum, sourceProp, EdgeMetaKey.PATH);
		...
		this.keysQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, typeSequenceNum, sourceProp,
				EdgeMetaKey.ROWKEYS);
		this.countQual = rowReader.getColumnKeyFactory()
				.createColumnKey(this.sourceType, typeSequenceNum, sourceProp,
						EdgeMetaKey.COUNT);
	...
	GraphEdgeReader(PlasmaType targetType, PlasmaProperty sourceProp,
		...
		this.family = tableConfig.getDataColumnFamilyNameBytes();
		this.baseTypeQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, sourceProp, EdgeMetaKey.BASETYPE);
		this.pathQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, sourceProp, EdgeMetaKey.PATH);
		this.sequencesQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, sourceProp, EdgeMetaKey.SEQUENCES);
		this.subTypeQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, sourceProp, EdgeMetaKey.SUBTYPE);
		// this.tableQual = rowReader.getColumnKeyFactory().createColumnKey(
		// this.sourceType, sourceProp, EdgeMetaField._TBL_);
		this.keysQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, sourceProp, EdgeMetaKey.ROWKEYS);
		this.countQual = rowReader.getColumnKeyFactory().createColumnKey(
				this.sourceType, sourceProp, EdgeMetaKey.COUNT);

		this.construct(rowReader, graphConfig);
	}

	static boolean exists(PlasmaType targetType, PlasmaProperty sourceProp,
			TableConfig tableConfig, DataGraphConfig graphConfig,
			RowReader rowReader) throws IOException {
		byte[] fam = tableConfig.getDataColumnFamilyNameBytes();
		byte[] typeQual = rowReader.getColumnKeyFactory().createColumnKey(
				targetType, sourceProp, EdgeMetaKey.BASETYPE);
	...
	static boolean exists(PlasmaType targetType, PlasmaProperty sourceProp,
		...
		byte[] fam = tableConfig.getDataColumnFamilyNameBytes();
		byte[] typeQual = rowReader.getColumnKeyFactory().createColumnKey(
				targetType, typeSequenceNum, sourceProp, EdgeMetaKey.BASETYPE);
		return rowReader.getRow().containsColumn(fam, typeQual);
	}
	...
	private void construct(RowReader rowReader, DataGraphConfig graphConfig) {
		...
		byte[] sequenceBytes = findValue(family, sequencesQual,
				rowReader.getRow());
		if (sequenceBytes != null) {
			...
			if (count != this.sequences.size())
				throw new IllegalStateException("invalid value " + count
						...
						+ Bytes.toString(rowReader.getRowKey())
						+ "' - expected " + this.sequences.size());
		}
		...
		if (rowKeyBytes != null) {
			...
			if (count != this.rowKeys.size())
				throw new IllegalStateException("invalid value " + count
						...
						+ Bytes.toString(countQual) + ") for key: '"
						+ Bytes.toString(rowReader.getRowKey())
						+ "' - expected " + this.rowKeys.size());
		}

		byte[] subtypesBytes = findValue(family, subTypeQual,
				rowReader.getRow());

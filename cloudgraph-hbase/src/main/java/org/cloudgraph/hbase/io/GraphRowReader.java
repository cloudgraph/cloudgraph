 ...
 * @since 0.5.1
 */
public class GraphRowReader extends DefaultRowOperation
	...
	public GraphRowReader(byte[] rowKey,
		...
		byte[] state = this.row.getColumnValue(
				this.tableReader.getTableConfig().getDataColumnFamilyNameBytes(), 
		...
		this.sequenceMapping = new BindingSequenceGenerator(Bytes.toString(state),
	...
	public EdgeReader getEdgeReader(PlasmaType type, PlasmaProperty property,
		...
		if (edgeReader == null) {
			if (sequence > 0) {
			    edgeReader = new EdgeReader(type, property, 
			    	sequence, 
			    	this.getTableReader().getTableConfig(), 
			    	this.graphConfig, 
					this);
			}
			else {				
	...
	public boolean edgeExists(PlasmaType type, PlasmaProperty property,
			long sequence) throws IOException {
		if (sequence > 0)
		    return EdgeReader.exists(type, property, sequence, 
				this.getTableReader().getTableConfig(), this.graphConfig, this);
		else
			return EdgeReader.exists(type, property, 
					this.getTableReader().getTableConfig(), this.graphConfig, this);
	}

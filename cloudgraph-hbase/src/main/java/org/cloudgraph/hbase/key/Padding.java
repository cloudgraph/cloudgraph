...
public class Padding {
     ...
     * with spaces depending on the given data flavor. Returns the given array as is
     * if the given array size is less than or equal to the given max length. 
    ...
    public byte[] back(byte[] src, int maxLength, byte pad) {
    	...
    	int delta = maxLength - src.length;
		System.arraycopy(src, 
        	0, result, 0, src.length);
    	Arrays.fill(result, src.length, 
    		src.length+delta, pad);
    	return result;
    }
    ...
    public String front(String src, int maxLength, char pad) {
    	...
    	int offset = delta;
		Arrays.fill(result, 0, delta, pad);
		System.arraycopy(srcchars, 
			0, result, offset, srcchars.length);
    	return new String(result);
    }
    ...
    public String back(String src, int maxLength, char pad) {
    	...
    	int delta = maxLength - srcchars.length;
		System.arraycopy(srcchars, 
        	0, result, 0, srcchars.length);
    	Arrays.fill(result, srcchars.length, 
    			srcchars.length+delta, pad);
    	return new String(result);
    }

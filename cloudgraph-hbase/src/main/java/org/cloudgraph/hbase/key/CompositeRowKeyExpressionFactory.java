		...
		for (KeyFieldConfig fieldConfig : this.getGraph().getRowKeyFields()) {
    		...
    		if (fieldConfig instanceof PreDefinedKeyFieldConfig) {
    			...
    			if (found != null) {
    				...
    				if (userFieldConfig.isHash()) {
    					if (found.isWildcard())
    						throw new GraphKeyException("cannot create wildcard expression for user"  
    							...
    							+ " for table '"
    							+ this.getTable().getName() + "' - this field is defined as using an integral hash algorithm which prevents the use of wildcards");
    				}
    				...
    				if (userFieldConfig.isHash()) 
						throw new GraphKeyException("cannot default datatype expression for user"  
							+ " defined row-key field with XPath expression '" 
							+ userFieldConfig.getPathExpression() + "'"
							+ " for table '"
							+ this.getTable().getName() + "' - this field is defined as using an integral hash algorithm which prevents the use of wildcards");

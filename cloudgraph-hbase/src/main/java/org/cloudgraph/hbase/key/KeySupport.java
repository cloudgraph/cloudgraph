...
public class KeySupport {
	private static final Log log = LogFactory
			.getLog(CompositeRowKeyFactory.class);

	/**
	 * Returns the specific configured hash algorithm configured for an HTable,
	 * or if not configured returns the configured HBase hash algorithm as
	 * configured within HBase using the 'hbase.hash.type' property.
	 * 
	...
	public Hash getHashAlgorithm(TableConfig table) {
		...
		if (table.hasHashAlgorithm()) {
			String hashName = table.getTable().getHashAlgorithm().getName()
					.value();
	...
	public KeyValue findKeyValue(UserDefinedRowKeyFieldConfig fieldConfig,
			List<KeyValue> pairs) {
		...
		for (KeyValue keyValue : pairs) {
			if (keyValue.getProp().getName()
					.equals(fieldConfig.getEndpointProperty().getName())) {
				if (keyValue.getProp().getContainingType().getName()
						.equals(fieldPropertyType.getName())) {
					if (keyValue.getProp().getContainingType().getURI()
							.equals(fieldPropertyType.getURI())) {
						if (fieldConfig.getPropertyPath() != null) {
							if (keyValue.getPropertyPath() != null
									&& keyValue.getPropertyPath().equals(
	 ...
	 *            the SDO Type
	 * @param hash
	 *            the hash algorithm to use in the event the row key token is to
	 *            be hashed
	 * @param token
		...
		switch (token.getName()) {
			...
			default :
				throw new CloudGraphConfigurationException(
						"invalid row key token name, " + token.getName().name()
	...
	public byte[] getPredefinedFieldValueStartBytes(PlasmaType type,
			Hashing hashing, PreDefinedKeyFieldConfig token) {
		return getPredefinedFieldValueBytes(type, hashing, token);
	}

	public byte[] getPredefinedFieldValueStopBytes(PlasmaType type,
			Hashing hashing, PreDefinedKeyFieldConfig token) {
		byte[] result = null;
		switch (token.getName()) {
			case URI :
	 ...
	 * @return the token value
	 */
	public byte[] getPredefinedFieldValueBytes(DataGraph dataGraph, Hash hash,
			PredefinedField token) {
		return getPredefinedFieldValueBytes(dataGraph.getRootObject(), hash,
				token);
	}
	 ...
	 * @return the token value
	 */
	public byte[] getPredefinedFieldValueBytes(DataObject dataObject,
			Hash hash, PredefinedField token) {
		...
		switch (token.getName()) {
			...
			case TYPE :
				...
				break;
			case UUID :
				result = Bytes.toBytes(((PlasmaDataObject) dataObject)
						.getUUIDAsString());
				break;
			default :
				throw new CloudGraphConfigurationException(

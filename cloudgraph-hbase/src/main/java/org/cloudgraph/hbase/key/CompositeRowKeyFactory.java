...
public class CompositeRowKeyFactory extends ByteBufferKeyFactory
	...
	public String createRowKey(Type type) {
		...
		List<PreDefinedKeyFieldConfig> preDefinedFields = this.getGraph()
		...
		for (int i = 0; i < preDefinedFields.size(); i++) {
			...
			String tokenValue = this.keySupport.getPredefinedFieldValue(
					plasmaType, this.hashing, preDefinedField);

			String padded = this.padding.pad(tokenValue,
	...
	private void create(PlasmaType type) {
		...
		for (int i = 0; i < preDefinedFields.size(); i++) {
			...
			byte[] tokenValue = this.keySupport.getPredefinedFieldValueBytes(
					type, this.hashing, preDefinedField);
		...
		for (KeyFieldConfig fieldConfig : this.getGraph().getRowKeyFields()) {
			...
			if (fieldConfig.isHash()) {
				...
				paddedKeyValue = padding.pad(keyValue,
						fieldConfig.getMaxLength(), DataFlavor.integral);
			} else {
	...
	public byte[] createRowKeyBytes(List<KeyValue> values) {
		...
		for (KeyFieldConfig fieldConfig : this.getGraph().getRowKeyFields()) {
			...
			if (fieldConfig instanceof PreDefinedKeyFieldConfig) {
				...
				KeyValue keyValue = this.keySupport.findKeyValue(
						userFieldConfig, values);
			...
			if (fieldConfig.isHash()) {
				...
				if (fieldConfig instanceof UserDefinedRowKeyFieldConfig) {
					...
					paddedKeyValue = padding.pad(fieldValue, userField
							.getMaxLength(), userField.getEndpointProperty()
							.getDataFlavor());
				} else {
					paddedKeyValue = padding.pad(fieldValue,
							fieldConfig.getMaxLength(),

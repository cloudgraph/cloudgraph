...
/**
 * Creates an HBase value and qualifier filter hierarchy to return HBase scan results 
 * representing a graph "slice". Accommodates slice predicates where 2 or more columns (qualifiers) are
 * involved. In such cases the qualifier filters must be grouped together in an 'OR' filter list 
 *  <a target="#" href="https://hbase.apache.org/apidocs/org/apache/hadoop/hbase/filter/FilterList.Operator.html#MUST_PASS_ONE">MUST_PASS_ONE</a>  
 *  and the value filters must, for example be grouped separately an another filter list. 
 * 
 ...
 * @see org.cloudgraph.hbase.filter.MultiColumnPredicateVisitor
 * @author Scott Cinnamond
 * @since 0.6.5
 */
public class MultiColumnPredicateFilterAssembler extends MultiColumnPredicateVisitor
    implements PredicateFilterAssembler
{
     ...
     */
	public MultiColumnPredicateFilterAssembler( 
			PlasmaType rootType) 
	{
		super(rootType);

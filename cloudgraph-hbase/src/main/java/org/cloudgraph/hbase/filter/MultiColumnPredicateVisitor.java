...
/**
 * Creates an HBase column filter hierarchy using <a target="#" href=
...
public class MultiColumnPredicateVisitor extends PredicateVisitor {
	private static Log log = LogFactory
			.getLog(MultiColumnPredicateVisitor.class);
	...
	protected FilterList qualifierFilterList = new FilterList(
			FilterList.Operator.MUST_PASS_ONE);
	protected FilterList valueFilterList = new FilterList(
			FilterList.Operator.MUST_PASS_ALL);

	public MultiColumnPredicateVisitor(PlasmaType rootType) {
		super(rootType);
	}
	 ...
	 * default {@link Operator#MUST_PASS_ALL AND} operator and pushes it onto
	 * the stack. Any subsequent {@link org.plasma.query.model.Literal literals}
	...
	public void start(Property property) {
		...
		if (path != null) {
			for (int i = 0; i < path.getPathNodes().size(); i++) {
				...
				PlasmaProperty prop = (PlasmaProperty) targetType
						.getProperty(namedPathElem.getValue());
		...
		byte[] colKey = this.columnKeyFac.createColumnKey(this.contextType,
		...
		if (qualifierFilterList.getFilters().size() == 1) // first one
			this.filterStack.peek().addFilter(qualifierFilterList);
	...
	public void start(Literal literal) {
		...
		if (this.contextQueryProperty == null)
			throw new IllegalStateException(
					"expected context query property for literal");
		...
		if (!this.contextOpWildcard) {
			...
			valueFilter = new ValueFilter(this.contextHBaseCompareOp,
					new BinaryComparator(valueBytes));
		} else {
			if (!validateWildcardDataFlavor(this.contextProperty
					.getDataFlavor()))
				throw new InvalidOperatorException(this.contextWildcardOperator
						.getValue().name(),
						this.contextProperty.getDataFlavor());
			String replaceExpr = "\\" + QueryConstants.WILDCARD;
			String expr = getDataFlavorRegex(this.contextProperty
					.getDataFlavor());
			String contentExpr = content.replaceAll(replaceExpr, expr);
			valueFilter = new ValueFilter(this.contextHBaseCompareOp,
					new RegexStringComparator(contentExpr));
		}

		valueFilterList.addFilter(valueFilter);

		if (valueFilterList.getFilters().size() == 1) // first one
			this.filterStack.peek().addFilter(valueFilterList);
	...
	public void start(LogicalOperator operator) {
		...
		switch (operator.getValue()) {
			case AND :
				// FilterList andList = new FilterList(
				// FilterList.Operator.MUST_PASS_ALL);
				// if (this.filterStack.size() > 0) {
				// FilterList top = this.filterStack.peek();
				// top.addFilter(andList);
				// }
				// else {
				// this.filterStack.push(andList);
				// }

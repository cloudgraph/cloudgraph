...
class GraphSliceSupport {
	 ...
	 * from the same path predicate. Each sub-graph must first be assembled to
	 * do any evaluation, but a single syntax tree instance evaluates every
	 * sub-graph (potentially thousands/millions) resulting from the given edge
	 * collection. The graph {@link Selection selection criteria} is based not
	 * on the primary graph selection but only on the properties found in the
	 * given path predicate, so the assembly is only/exactly as extensive as
	 * required by the predicate. Any sub-graphs assembled may themselves be
	 * "distributed" graphs.
	 * 
	 * @param contextType
	 *            the current type
	 ...
	 * @see Selection
	 */
	public Map<String, Result> filter(PlasmaType contextType,
			EdgeOperation collection, Where where, RowReader rowReader,
			TableReader tableReader) throws IOException {
		Map<String, Result> results = new HashMap<String, Result>();

		SelectionCollector selectionCollector = new SelectionCollector(where,
				contextType);
		...
		DistributedReader existingReader = (DistributedReader) tableReader
		...
		HBaseGraphAssembler graphAssembler = new GraphAssembler(contextType,
				selectionCollector, sliceGraphReader, snapshotDate);
		...
		DataGraphConfig graphConfig = CloudGraphConfig.getInstance()
				.getDataGraph(contextType.getQualifiedName());
	 ...
	 * the column qualifier sequence numbers which represent the subset of total
	 * graph edges as restricted by the predicate.
	 * 
	 * @param contextType
	 ...
	 */
	public Set<Long> fetchSequences(PlasmaType contextType, Where where,
		...
		boolean multiDescendantProperties = predicateUtil
				.hasHeterogeneousDescendantProperties(where);
		...
		Result result = fetchResult(get, rowReader.getTableReader(),
				graphConfig);
		Map<Long, Map<String, KeyValue>> buckets = buketizeResult(result,
				graphConfig);
		...
		EdgeRecognizerSyntaxTreeAssembler assembler = new EdgeRecognizerSyntaxTreeAssembler(
	...
	public Result[] fetchResult(List<Get> gets, TableOperation tableOperation,
		...
		if (log.isDebugEnabled())
			try {
				log.debug("get filter: "
						+ FilterUtil.printFilterTree(gets.get(0).getFilter()));
			} catch (IOException e1) {
			}
	...
	public Map<Long, Map<String, KeyValue>> buketizeResult(Result result,
			DataGraphConfig graphConfig) {
		...
		if (!result.isEmpty())
			for (KeyValue keyValue : result.list()) {
				...
				String[] sections = qual.split(graphConfig
						.getColumnKeySequenceDelimiter());
	...
	/**
	 * Runs the given get and parses the column qualifier sequence number
	 ...
	 * @throws IOException
	 */
	public Map<Integer, Integer> fetchSequences(Get get,
			TableReader tableReader, DataGraphConfig graphConfig)
			throws IOException {
		if (log.isDebugEnabled())
			try {
				log.debug("get filter: "
	 ...
	 */
	public void load(Set<Property> properties, PlasmaType contextType,
			RowReader rowReader) throws IOException {
		Get get = new Get(rowReader.getRowKey());
	 ...
	 * @throws IOException
	 */
	public void loadBySequenceList(Set<Long> sequences,
			Set<Property> properties, PlasmaType contextType,
			RowReader rowReader, EdgeReader edgeReader) throws IOException {
		Get get = new Get(rowReader.getRowKey());
		PlasmaType rootType = (PlasmaType) rowReader.getRootType();

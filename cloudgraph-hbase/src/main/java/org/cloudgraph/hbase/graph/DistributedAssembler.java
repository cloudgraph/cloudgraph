    ...
    implements HBaseGraphAssembler
{
     ...
     * </p>
	 * @param resultRow the HBase client result (row).
	 */
	@Override
	 ...
	 * @param targetSequence the current data object sequence
	...
	protected abstract void assemble( 
		PlasmaDataObject target, long targetSequence, EdgeReader collection,	
	...
	protected void assembleEdge(long childSequence, EdgeReader collection, RowReader childRowReader, 
		PlasmaDataObject source, long sourceSequence, PlasmaProperty sourceProperty, int level) throws IOException {
		...
		//assemble(child, childSequence, target, prop, childRowReader, level+1);		
	}	
	...
	{	    	
		...
		UUID uuid = fetchRootUUID(childTableReader, childResult);	
		PlasmaType childType = this.fetchType(childTableReader, childResult);		
		...
		RowReader childRowReader = childTableReader.createRowReader(
			child, childResult);
		...
		assemble(child, CloudGraphConstants.ROOT_SEQUENCE, collection, source, sourceProperty, childRowReader, level+1);

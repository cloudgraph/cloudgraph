...
public abstract class DistributedAssembler extends DefaultAssembler
	...
	public DistributedAssembler(PlasmaType rootType, Selection selection,
		...
		super(rootType, selection, distributedReader.getRootTableReader(),
				snapshotDate);
	...
	public void assemble(Result resultRow) {
		...
		RowReader rowReader = this.rootTableReader.createRowReader(this.root,
				resultRow);
		...
		root.getValueObject().put(CloudGraphConstants.ROW_KEY,
				rowReader.getRowKey());
		root.getValueObject().put(CloudGraphConstants.SEQUENCE,
				CloudGraphConstants.ROOT_SEQUENCE);

		long before = System.currentTimeMillis();
		try {
			assemble(this.root, CloudGraphConstants.ROOT_SEQUENCE, null, null,
					null, rowReader, 0);
		} catch (IOException e) {
		...
		root.getValueObject().put(CloudGraphConstants.GRAPH_ASSEMBLY_TIME,
		...
		root.getValueObject().put(CloudGraphConstants.GRAPH_DEPTH,
		...
		root.getValueObject()
				.put(CloudGraphConstants.GRAPH_TABLE_NAMES, tables);
	}
	...
	protected abstract void assemble(PlasmaDataObject target,
			long targetSequence, EdgeReader collection,
			PlasmaDataObject source, PlasmaProperty sourceProperty,
			RowReader rowReader, int level) throws IOException;

	protected void assembleEdge(long childSequence, EdgeReader collection,
			RowReader childRowReader, PlasmaDataObject source,
			long sourceSequence, PlasmaProperty sourceProperty, int level)
			throws IOException {
		...
		UUID uuid = this.fetchUUID(subType, childSequence, collection,
				childRowReader);
		PlasmaType childType = this.fetchType(subType, childSequence,
				collection, childRowReader);
		PlasmaDataObject child = createChild(uuid, childType, childSequence,
		...
		if (!childRowReader.contains(child, childSequence,
				(PlasmaType) child.getType()))
			childRowReader.addDataObject(child, childSequence,
					(PlasmaType) child.getType());

		// childRowReader.addDataObject(child);
		// add it after minimally created with UUID

		assemble(child, childSequence, collection, source, sourceProperty,
	...
	protected void assembleExternalEdge(String childRowKey,
			EdgeReader collection,
			/* RowReader childRowReader, */TableReader childTableReader,
			PlasmaDataObject source, long sourceSequence,
		...
		Result childResult = fetchGraph(Bytes.toBytes(childRowKey),
				childTableReader, subType);
		...
		UUID uuid = this.fetchRootUUID(childTableReader, this.rootKeyFactory,
				subType, childResult);
		PlasmaType childType = this.fetchRootType(childTableReader,
				this.rootKeyFactory, subType, childResult);

		// create a child object using UUID from external row root
		PlasmaDataObject child = createChild(source, sourceProperty, uuid,
				childType);
		if (log.isDebugEnabled())
			log.debug("initialized external child: " + child);

		RowReader childRowReader = childTableReader.createRowReader(child,
				childResult);
		...
		assemble(child, CloudGraphConstants.ROOT_SEQUENCE, collection, source,
				sourceProperty, childRowReader, level + 1);

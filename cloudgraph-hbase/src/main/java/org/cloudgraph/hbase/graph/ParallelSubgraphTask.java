...
class ParallelSubgraphTask extends DefaultSubgraphTask implements SubgraphTask {
	 ...
	 */
	public ParallelSubgraphTask(PlasmaDataObject subroot, long subrootSequence,
			...
			RowReader rowReader, int level, int taskSequence,
		...
		super(subroot, subrootSequence, selection, snapshotDate,
				...
				rowReader, level, taskSequence, executorService, config);
	}
	...
	protected SubgraphTask newTask(PlasmaDataObject subroot,
			...
			ThreadPoolExecutor executorService, ConfigProps config) {
		return new ParallelSubgraphTask(subroot, subrootSequence, selection,
				snapshotDate, distributedReader, collection, source,
		...
		for (Property p : props) {
			...
			if (!edgeReader.isExternal()) {
				assembleEdges(target, targetSequence, prop, edgeReader,
						rowReader, tableReader, rowReader, level);
			} else {
				// String childTable =
	...
	protected void assembleEdges(PlasmaDataObject target, long targetSequence,
			PlasmaProperty prop, EdgeReader collection, RowReader rowReader,
			TableReader childTableReader, RowReader childRowReader, int level)
			throws IOException {
		for (long childSequence : collection.getSequences()) {
			...
			if (childRowReader.contains(childSequence, subType)) {
				...
				PlasmaDataObject existingChild = (PlasmaDataObject) childRowReader
						.getDataObject(childSequence, collection.getBaseType());
			...
			PlasmaDataObject child = null;
			synchronized (target) {
				child = createChild(childSequence, collection, source,
	 ...
	 * within this table or another. Since we are assembling a graph, and each
	 * edge links another row, each edge requires a new row reader.
	 * 
	 * @param target
	...
	protected void assembleExternalEdges(PlasmaDataObject target,
			...
			RowReader rowReader, TableReader childTableReader, int level)
			throws IOException {
		for (String childRowKey : collection.getRowKeys()) {
			...
			if (log.isDebugEnabled())
				log.debug("external edge: " + target.getType().getURI() + "#"
						+ target.getType().getName() + "->" + prop.getName()
						+ " (" + childRowKey + ")");

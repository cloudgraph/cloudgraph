 ...
 */
// package protection
class ParallelSliceSubgraphTask extends DefaultSubgraphTask
		implements
			SubgraphTask {
	...
	public ParallelSliceSubgraphTask(PlasmaDataObject subroot,
			long subrootSequence, Selection selection, Timestamp snapshotDate,
			DistributedReader distributedReader, EdgeReader collection,
			PlasmaDataObject source, PlasmaProperty sourceProperty,
			RowReader rowReader, int level, int sequence,
			ThreadPoolExecutor executorService, ConfigProps config) {
		super(subroot, subrootSequence, selection, snapshotDate,
				distributedReader, collection, source, sourceProperty,
				rowReader, level, sequence, executorService, config);
	...
	protected SubgraphTask newTask(PlasmaDataObject subroot,
			...
			RowReader rowReader, int level, int sequence,
			ThreadPoolExecutor executorService, ConfigProps config) {
		return new ParallelSliceSubgraphTask(subroot, subrootSequence,
				selection, snapshotDate, distributedReader, collection, source,
				sourceProperty, rowReader, level, sequence, executorService,
				config);
	}
	...
	protected void assemble(PlasmaDataObject target, long targetSequence,
		...
		Set<Property> props = this.getProperties(target, source,
				sourceProperty, level);
		if (props.size() == 0)
			return;
		if (log.isDebugEnabled())
			log.debug("assembling(" + level + "): " + target.toString() + ": "
					+ props.toString());
		...
		for (Property p : props) {
			...
			// if (level > 0) {
			if (rowReader.edgeExists((PlasmaType) target.getType(), prop,
					targetSequence)) {
				edgeReader = rowReader.getEdgeReader(
						(PlasmaType) target.getType(), prop, targetSequence);
			} else
			...
			if (!edgeReader.isExternal()) {
				...
				if (prop.isMany() && where != null) {
					sequences = this.sliceSupport.fetchSequences(
							(PlasmaType) prop.getType(), where, rowReader);
					// preload properties for the NEXT level into the current
					// row so we have something to assemble
					Set<Property> childProperies = this.selection
							.getInheritedProperties(prop.getType(), level + 1);
					this.sliceSupport.loadBySequenceList(sequences,
							childProperies, childType, rowReader, edgeReader);
				} else {
					// preload properties for the NEXT level into the current
					// row so we have something to assemble
					Set<Property> childProperies = this.selection
							.getInheritedProperties(prop.getType(), level + 1);
					this.sliceSupport
							.load(childProperies, childType, rowReader);
				}

				assembleEdges(target, targetSequence, prop, edgeReader,
						sequences, rowReader, rowReader.getTableReader(),
						rowReader, level);
			} else {
				// String childTable =
				...
				TableReader externalTableReader = distributedReader
						.getTableReader(edgeReader.getTable());
				if (log.isDebugEnabled())
					if (!tableConfig.getName().equals(
							externalTableReader.getTableConfig().getName()))
						log.debug("switching row context from table: '"
								+ tableConfig.getName()
								+ "' to table: '"
								+ externalTableReader.getTableConfig()
										.getName() + "'");
				Map<String, Result> resultRows = null;
				if (prop.isMany() && where != null) {
					resultRows = this.sliceSupport.filter(childType,
							edgeReader, where, rowReader, externalTableReader);
				}
				assembleExternalEdges(target, targetSequence, prop, edgeReader,
						rowReader, resultRows, externalTableReader, level);
			}
		}
	...
	private void assembleEdges(PlasmaDataObject target, long targetSequence,
			PlasmaProperty prop, EdgeReader edgeReader, Set<Long> sequences,
			RowReader rowReader, TableReader childTableReader,
		...
		for (long childSequence : edgeReader.getSequences()) {
			...
			if (childRowReader.contains(childSequence, subType)) {
				...
				// link
				PlasmaDataObject existingChild = (PlasmaDataObject) childRowReader
						.getDataObject(childSequence, edgeReader.getBaseType());
			...
			if (log.isDebugEnabled())
				log.debug("local edge: " + target.getType().getURI() + "#"
						+ target.getType().getName() + "->" + prop.getName()
						+ " (" + childSequence + ")");
			synchronized (this.distributedReader) {
				this.distributedReader.mapRowReader(childSequence,
						edgeReader.getBaseType(), childRowReader);
			}
			...
			synchronized (target) {
				child = createChild(childSequence, edgeReader, source,
						sourceProperty, childRowReader, this.graph);
			}
			...
			traversals.add(new Traversal(child, childSequence, edgeReader,
					target, prop, childRowReader, false, // indicate a
															// non-concurrent
															// traversal
					level + 1));
		}
	...
	/**
	 * Assembles a given set of edges where the target is a different row,
	 * within this table or another. Since we are assembling a graph, each edge
	 * requires a new row reader. Each edge is a new root in the target table so
	 * need a new row reader for each.
	 * 
	 * @param target
	 ...
	 * @throws IOException
	 */
	protected void assembleExternalEdges(PlasmaDataObject target,
			long targetSequence, PlasmaProperty prop, EdgeReader collection,
			RowReader rowReader, Map<String, Result> resultRows,
		...
		for (String childRowKey : collection.getRowKeys()) {
			...
			RowReader existingChildRowReader = childTableReader
					.getRowReader(childRowKey);
			...
			if (log.isDebugEnabled())
				log.debug("fetch external row: " + prop.toString() + " ("
						+ childRowKey + ")");

			childResult = fetchGraph(Bytes.toBytes(childRowKey),
					childTableReader, collection.getBaseType());
			...
			byte[] uuidQual = this.rootKeyFactory.createColumnKey(subType,
					EntityMetaKey.UUID);
			// need to reconstruct the original graph, so need original UUID
			byte[] rootUuid = childResult.getValue(Bytes
					.toBytes(childTableReader.getTableConfig()
							.getDataColumnFamilyName()), uuidQual);
			if (rootUuid == null)
				throw new GraphServiceException("expected column: "
						+ childTableReader.getTableConfig()
								.getDataColumnFamilyName() + ":"
						+ EntityMetaKey.UUID);
			String uuidStr = null;
			uuidStr = new String(rootUuid, childTableReader.getTableConfig()
					.getCharset());
			...
			synchronized (target) {
				...
				child = createChild(source, sourceProperty, uuid,
			...
			synchronized (childTableReader) {
				...
				childRowReader = childTableReader.createRowReader(child,
						childResult);
			}
			synchronized (this.distributedReader) {
				this.distributedReader
						.mapRowReader(childRowKey, childRowReader);
			}
			...
			traversals.add(new Traversal(child, -1, collection, target, prop,

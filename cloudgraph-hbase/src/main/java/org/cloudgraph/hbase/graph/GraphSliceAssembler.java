...
public class GraphSliceAssembler extends DistributedAssembler {
		...
		for (Property p : props) {
			...
			if (!edgeReader.isExternal()) {
				...
				if (prop.isMany() && where != null) {
					sequences = this.slice.fetchSequences(
					...
					this.slice.loadBySequenceList(sequences, childProperies,
				...
				if (prop.isMany() && where != null) {
					resultRows = this.slice.filter(childType, edgeReader,
		...
		for (long sequence : sequences) {
			...
			if (childRowReader.contains(sequence, subType)) {
				...
				PlasmaDataObject existingChild = (PlasmaDataObject) childRowReader
						.getDataObject(sequence, subType);
			...
			this.assembleEdge(sequence, edgeReader, childRowReader, target,
					targetSequence, prop, level);
		}
	}
	...
	private void assembleExternalEdges(PlasmaDataObject target,
		...
		for (String childRowKey : collection.getRowKeys()) {
			...
			this.assembleExternalEdge(childRowKey, collection,
					childTableReader, target, targetSequence, prop, level);
			i++;
		}
	}

...
public class GraphAssembler extends DistributedAssembler {
	...
	protected void assemble(PlasmaDataObject target, long targetSequence,
		...
		if (log.isDebugEnabled())
			log.debug("assembling(" + level + "): " + target + ": "
					+ props.toString());
	...
	protected void assembleEdges(PlasmaDataObject source, long sourceSequence,
			PlasmaProperty sourceProp, EdgeReader collection,
		...
		for (long sequence : collection.getSequences()) {
			...
			if (log.isDebugEnabled())
				log.debug("reading local edge: " + source.getType() + "->"
						+ sourceProp.getName() + " (" + sequence + ")");
			...
			if (childRowReader.contains(sequence, subType)) {
				...
				PlasmaDataObject existingChild = (PlasmaDataObject) childRowReader
						.getDataObject(sequence, collection.getBaseType());
				if (log.isDebugEnabled())
					log.debug("linking existing local child (sequence: "
							+ sequence + "): " + existingChild);
			...
			this.assembleEdge(sequence, collection, childRowReader, source,
					sourceSequence, sourceProp, level);
		}
	}
	 ...
	 * @throws IOException
	 */
	protected void assembleExternalEdges(PlasmaDataObject source,
			long sourceSequence, PlasmaProperty sourceProp,
		...
		for (String childRowKey : collection.getRowKeys()) {
			if (log.isDebugEnabled())
				log.debug("reading external edge: " + source.getType() + "->"
						+ sourceProp.getName() + "key ('" + childRowKey + "')");
			...
			if (existingChildRowReader != null) {
				...
				PlasmaDataObject existingChild = (PlasmaDataObject) existingChildRowReader
						.getRootDataObject();
				if (log.isDebugEnabled())
					log.debug("linking existing external child (key: "
							+ childRowKey + "): " + existingChild);
				link(existingChild, source, sourceProp);
				continue;
			}

			this.assembleExternalEdge(childRowKey, collection,
					childTableReader, source, sourceSequence, sourceProp, level);
		}
	}

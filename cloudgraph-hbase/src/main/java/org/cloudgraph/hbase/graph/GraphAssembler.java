...
//							edges[0].getType());
	...
	protected void assembleEdges(PlasmaDataObject source, long sourceSequence, PlasmaProperty sourceProp, 
		...
		int level) throws IOException 
	{
		...
		for (long sequence : collection.getSequences()) {
        	...
        	{            		
        		...
        		PlasmaDataObject existingChild = (PlasmaDataObject)childRowReader.getDataObject(sequence, collection.getBaseType());
       		    if (log.isDebugEnabled())
    				log.debug("linking existing local child (sequence: "+sequence+"): " 
    				        + existingChild);
...
//            childRowReader.addDataObject(child);
        	...
        	this.assembleEdge(sequence, collection, childRowReader, source, sourceSequence, sourceProp, level);
		}
	}
	 ...
	 * @param source the object source to which we link edges
	 * @param sourceProp the edge property
	 ...
	 * @throws IOException
	 */
	protected void assembleExternalEdges(PlasmaDataObject source, long sourceSequence, PlasmaProperty sourceProp, 
			EdgeReader collection, RowReader rowReader, TableReader childTableReader, int level) throws IOException 
	{
		for (String childRowKey : collection.getRowKeys()) {
			if (log.isDebugEnabled())
				log.debug("reading external edge: " 
			        + source.getType()
        	...
        	{      
        		...
        		PlasmaDataObject existingChild = (PlasmaDataObject)existingChildRowReader.getRootDataObject();
       		    if (log.isDebugEnabled())
    				log.debug("linking existing external child (key: "+childRowKey+"): " 
    				        + existingChild);
    		    link(existingChild, source, sourceProp);
    		    continue; 
        	}			
			
			this.assembleExternalEdge(childRowKey, collection, childTableReader, 
					source, sourceSequence, sourceProp, level);
		}
	}

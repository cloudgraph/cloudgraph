...
//        	RowWriter rowWriter = graphWriter.getRowWriter(dataObject);        	
//        	rowWriter.getSequenceMapping().addSequence(dataObject);
//        }
	...
	private void collectRowMutations(TableWriter tableWriter, RowWriter rowWriter, List<Row> rowMutations) throws IOException
	{
	...
	private void addRemoveStateColumns(TableWriter tableWriter, RowWriter rowWriter) throws IOException
	{
		...
		if (!rowWriter.isRootDeleted()) {
			String xml = rowWriter.getSequenceMapping().marshal();
	    	if (log.isDebugEnabled())

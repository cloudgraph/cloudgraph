...
public class Delete extends DefaultMutation implements Collector   {
            ...
            RowWriter rowWriter) throws IOException, IllegalAccessException
    {
        ...
        Timestamp snapshotDate = (Timestamp)coreNode.getValue(CoreConstants.PROPERTY_NAME_SNAPSHOT_TIMESTAMP);                                     
        ...
        if (sequence == null)                                                                    
            throw new RequiredPropertyException("instance property '" + CloudGraphConstants.SEQUENCE                
               + "' is required to delete data object, " 
               + dataObject); 
        if (log.isDebugEnabled())
        ...
        // If no tombstones, and if the root of the row is deleted if this object is the root, delete it, else if
        // the given data object is part of the row, just return.  
        // No need to delete its individual properties, as the row is going away anyway. 
        ...
        // For tombstone rows, we want to delete app columns (and leave a tombstone column
        ...
        for (Property p : properties)
        {
        	...
        	if (property.getType().isDataType()) {
        	    ...
        	    //this.deleteDataCell(rowWriter, dataObject, sequence, property);
        	}
        	else {

...
abstract class DefaultMutation {
	...
	public DefaultMutation(ServiceContext context, SnapshotMap snapshotMap,
			String username) {
		super();
	...
	protected HashMap<String, DataObject> getOldEdgeMap(Object oldValue,
			Property property) {
	...
	/**
	 * Screens out edges that are not owned by the given data node, but still
	 * may be present.
	 */
	@Deprecated
	protected List<PlasmaEdge> findOwnedEdges(PlasmaNode dataNode,
			Property property, List<PlasmaEdge> edges,
			DistributedWriter graphWriter, RowWriter rowWriter)
			throws IOException {
		...
		for (PlasmaEdge edge : edges) {
			PlasmaDataObject opposite = edge.getOpposite(dataNode)
					.getDataObject();
			PlasmaType oppositeType = (PlasmaType) opposite.getType();
			boolean oppositeTypeBound = CloudGraphConfig.getInstance()
					.findTable(oppositeType.getQualifiedName()) != null;
			RowWriter oppositeRowWriter = graphWriter.findRowWriter(opposite);
			if (oppositeRowWriter == null) {
				oppositeRowWriter = graphWriter.createRowWriter(opposite);
			}
	...
	protected void updateCell(RowWriter rowContext,
			PlasmaDataObject dataObject, long sequence, Property property,
		...
		if (log.isDebugEnabled()) {
			if (prop.getType().isDataType()) {
				Object objectValue = HBaseDataConverter.INSTANCE.fromBytes(
						prop, value);
		...
		rowContext.getRow().add(table.getDataColumnFamilyNameBytes(),
				qualifier, value);
	}
	 ...
	 * @throws IOException
	 */
	protected void deleteDataCell(RowWriter rowContext,
			PlasmaDataObject dataObject, long sequence, PlasmaProperty property)
			throws IOException {
		byte[] qualifier = rowContext.getColumnKeyFactory().createColumnKey(
				dataObject, sequence, property);
		TableConfig table = rowContext.getTableWriter().getTableConfig();
		if (log.isDebugEnabled())
			log.debug("deleting " + property + " / " + table.getName() + "."
					+ new String(qualifier, table.getCharset()));
		rowContext.getRowDelete().addColumns(
				table.getDataColumnFamilyNameBytes(), qualifier);
	}
	...
	protected void setOrigination(PlasmaDataObject dataObject, PlasmaType type) {
		...
		if (originationUserProperty != null) {
			if (!originationUserProperty.isReadOnly())
				...
				((CoreDataObject) dataObject).setValue(
						originationUserProperty.getName(), username); // FIXME:
																		// bypassing
																		// readonly
																		// modification
																		// detection
		...
		if (originationTimestampProperty != null) {
			Date dateSnapshot = new Date(this.snapshotMap.getSnapshotDate()
					.getTime());
			...
			if (!originationTimestampProperty.isReadOnly())
				...
				((CoreDataObject) dataObject).setValue(
						originationTimestampProperty.getName(), snapshot); // FIXME:
																			// bypassing
																			// readonly
																			// modification
																			// detection
	...
	protected void updateOrigination(PlasmaDataObject dataObject,
			long sequence, PlasmaType type, RowWriter rowContext)
			throws IOException {
		// FIXME - could be a reference to a user
		Property originationUserProperty = type.findProperty(
				ConcurrencyType.origination, ConcurrentDataFlavor.user);
		...
		PlasmaProperty originationTimestampProperty = (PlasmaProperty) type
				.findProperty(ConcurrencyType.origination,
		...
		if (originationTimestampProperty != null) {
			...
			Object snapshot = DataConverter.INSTANCE.convert(
					originationTimestampProperty.getType(), dateSnapshot);
			byte[] bytes = HBaseDataConverter.INSTANCE.toBytes(
					originationTimestampProperty, snapshot);
			rowContext.writeRowData(dataObject, sequence,
					originationTimestampProperty, bytes);
		} // else if (log.isDebugEnabled())
			// log.debug("could not find origination date property for type, " +
			// type + "#" + type.getName());
	}

	protected void setOptimistic(PlasmaDataObject dataObject, PlasmaType type,
		...
		if (concurrencyUserProperty == null) {
			...
			if (!concurrencyUserProperty.isReadOnly())
				...
				((CoreDataObject) dataObject).setValue(
						concurrencyUserProperty.getName(), username);
		}
		...
		if (concurrencyTimestampProperty == null) {
			...
			if (!concurrencyTimestampProperty.isReadOnly())
				...
				((CoreDataObject) dataObject).setValue(
						concurrencyTimestampProperty.getName(), snapshot);
		}
	}

	// FIXME: need CG configuration related to enabling and handling concurrency
	// checks
	protected void updateOptimistic(PlasmaDataObject dataObject, long sequence,
			PlasmaType type, RowWriter rowContext) throws IOException {
		PlasmaProperty concurrencyUserProperty = (PlasmaProperty) type
				.findProperty(ConcurrencyType.optimistic,
						ConcurrentDataFlavor.user);
		if (concurrencyUserProperty == null) {
			...
			byte[] bytes = HBaseDataConverter.INSTANCE.toBytes(
					concurrencyUserProperty, username);
			rowContext.writeRowData(dataObject, sequence,
					concurrencyUserProperty, bytes);
		}
		...
		if (concurrencyTimestampProperty == null) {
			...
			Object snapshot = DataConverter.INSTANCE.convert(
					concurrencyTimestampProperty.getType(), dateSnapshot);
			byte[] bytes = HBaseDataConverter.INSTANCE.toBytes(
					concurrencyTimestampProperty, snapshot);
			rowContext.writeRowData(dataObject, sequence,
					concurrencyTimestampProperty, bytes);
		}
	}

	protected void validateModifications(DataGraph dataGraph,
			PlasmaDataObject dataObject, RowWriter rowWriter)
		...
		PlasmaType rootType = (PlasmaType) rowWriter.getRootDataObject()
				.getType();
		DataGraphConfig dataGraphConfig = CloudGraphConfig.getInstance()
				.getDataGraph(rootType.getQualifiedName());
		...
		for (Property p : properties) {
			...
			if (userDefinedField != null) {
				throw new IllegalAccessException(
						"attempt to modify row-key property, "
								+ type.getURI()
								+ "#"
								+ type.getName()
								+ "."
	...
	protected void checkConcurrency(DataGraph dataGraph,
			PlasmaDataObject dataObject) {
		PlasmaType type = (PlasmaType) dataObject.getType();
		...
		if (dataGraph.getChangeSummary().isCreated(dataObject)) {
			...
			if (snapshotDate == null)
				throw new RequiredPropertyException("instance property '"
						...
						+ "' is required to update entity " + dataObject);
	...
	// FIXME: need CG configuration related to enabling and handling concurrency
	// checks
	protected void checkOptimistic(PlasmaDataObject dataObject,
			PlasmaType type, Timestamp snapshotDate) {
		PlasmaProperty concurrencyUserProperty = (PlasmaProperty) type
				.findProperty(ConcurrencyType.optimistic,
						ConcurrentDataFlavor.user);
		if (concurrencyUserProperty == null) {
	...
	// FIXME: need CG configuration related to enabling and handling concurrency
	// checks
	protected void checkLock(PlasmaDataObject dataObject, PlasmaType type,
			Timestamp snapshotDate) {

...
//	protected byte[] createEdgeValueBytes(PlasmaNode dataNode, List<PlasmaEdge> edges, RowWriter rowWriter)
//			throws IOException {
...
//    }
	 ...
	 */
	@Deprecated
	protected List <PlasmaEdge> findOwnedEdges(
    	PlasmaNode dataNode,
    	Property property, 
    	List <PlasmaEdge> edges,
    	DistributedWriter graphWriter,
	...
	protected void updateCell(RowWriter rowContext, PlasmaDataObject dataObject, long sequence, Property property,
			byte[] value) throws IOException {
		PlasmaProperty prop = (PlasmaProperty) property;
		byte[] qualifier = rowContext.getColumnKeyFactory().createColumnKey((PlasmaType)dataObject.getType(), sequence, prop);
	 ...
	 * @throws IOException
	 */
	protected void deleteDataCell(RowWriter rowContext, PlasmaDataObject dataObject, long sequence, PlasmaProperty property) throws IOException {
		byte[] qualifier = rowContext.getColumnKeyFactory().createColumnKey(dataObject, sequence, property);
		TableConfig table = rowContext.getTableWriter().getTableConfig();
		if (log.isDebugEnabled())
...
//	}
	...
	protected void updateOrigination(PlasmaDataObject dataObject, long sequence, PlasmaType type, RowWriter rowContext)
			throws IOException {
		// FIXME - could be a reference to a user
		Property originationUserProperty = type.findProperty(ConcurrencyType.origination, ConcurrentDataFlavor.user);
		if (originationUserProperty != null) {
		...
		PlasmaProperty originationTimestampProperty = (PlasmaProperty)type.findProperty(ConcurrencyType.origination,
				ConcurrentDataFlavor.time);
		if (originationTimestampProperty != null) {
			Date dateSnapshot = new Date(this.snapshotMap.getSnapshotDate().getTime());
			Object snapshot = DataConverter.INSTANCE.convert(originationTimestampProperty.getType(), dateSnapshot);
	...
	// checks
	protected void updateOptimistic(PlasmaDataObject dataObject, long sequence, PlasmaType type, RowWriter rowContext)
			throws IOException {
		PlasmaProperty concurrencyUserProperty = (PlasmaProperty) type.findProperty(ConcurrencyType.optimistic,
				ConcurrentDataFlavor.user);
		if (concurrencyUserProperty == null) {

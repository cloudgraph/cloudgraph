...
public class IntegralLiteral extends ScanLiteral
	...
	public byte[] getEqualsStartBytes() {
		...
		Object value = this.dataConverter.convert(property.getType(),
				this.literal);
		String startValueStr = this.dataConverter.toString(property.getType(),
				value);
		if (this.fieldConfig.isHash()) {
			...
			paddedStartBytes = this.padding.pad(startBytes,
					this.fieldConfig.getMaxLength(), DataFlavor.integral);
		} else {
	...
	public byte[] getEqualsStopBytes() {
		...
		if (this.fieldConfig.isHash()) {
			String stopValueStr = this.dataConverter.toString(
					property.getType(), value);
			stopBytes = this.hashing
					.toStringBytes(stopValueStr, HASH_INCREMENT);
	...
	public byte[] getGreaterThanStartBytes() {
		...
		if (fieldConfig.isHash()) {
			String startValueStr = this.dataConverter.toString(
					property.getType(), value);
			startBytes = this.hashing.toStringBytes(startValueStr,
					HASH_INCREMENT);
			paddedStartBytes = this.padding.pad(startBytes,
	...
	/**
	 * The "less than" relational operator does not effect the start bytes for
	 * an HBase partial row-key scan and this method therefore returns an empty
	 * byte array or "no-op".
	 * 
	 * @return an empty byte array or "no-op".
	...
	public byte[] getLessThanStopBytes() {
		...
		String stopValueStr = this.dataConverter.toString(property.getType(),
				value);
		if (fieldConfig.isHash()) {
	...
	public byte[] getFuzzyKeyBytes() {
		...
		Object value = this.dataConverter.convert(property.getType(),
				this.literal);
		String valueStr = this.dataConverter
				.toString(property.getType(), value);
		if (fieldConfig.isHash()) {
	...
	private String increment(Type type, Object value) {
		...
		switch (sourceDataType) {
			case Short :
				Short shortValue = this.dataConverter.toShort(
						property.getType(), value);
				Short shortResult = Short.valueOf((short) (shortValue
						.shortValue() + INCREMENT));
				result = this.dataConverter.toString(type, shortResult);
				break;
			case Int :
				Integer intValue = this.dataConverter.toInt(property.getType(),
						value);
				Integer intResult = Integer.valueOf(intValue.intValue()
						+ INCREMENT);
				result = this.dataConverter.toString(type, intResult);
				break;
			case Long :
				Long longValue = this.dataConverter.toLong(property.getType(),
						value);
				Long longResult = Long.valueOf(longValue.longValue()
						+ INCREMENT);
				result = this.dataConverter.toString(type, longResult);
				break;
			case Integer :
				BigInteger integerValue = this.dataConverter.toInteger(
						property.getType(), value);
				BigInteger integerResult = integerValue.add(BigInteger
						.valueOf(INCREMENT));
				result = this.dataConverter.toString(type, integerResult);
				break;
			default :
				throw new ScanException(
						"expected integral (Float, Double, Decinal)datatype not, "
								+ sourceDataType.name());
		}
		return result;
	}

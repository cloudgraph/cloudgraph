...
public class WildcardStringLiteral extends StringLiteral
		...
		if (!fieldConfig.isHash()) {
			if (keyValueStr.endsWith(Wildcard.WILDCARD_CHAR)) {
				...
				throw new ScanException("cannot create fuzzy scan literal "
						...
						+ "' within table " + this.table.getName()
			...
			throw new ScanException("cannot create fuzzy scan literal "
					...
					+ this.table.getName() + " for graph root type, "
	 ...
	 *         a "wildcard" operator under an HBase partial row-key scan under
	 *         the various optionally configurable hashing, formatting and
	 *         padding features.
	 */
	@Override
	public byte[] getBetweenStartBytes() {
		byte[] startBytes = null;
		String startValueStr = this.literal.substring(0,
	...
	public byte[] getBetweenStopBytes() {
		byte[] stopBytes = null;
		String stopValueStr = this.literal.substring(0,
				this.literal.length() - 1);
		if (!fieldConfig.isHash()) {

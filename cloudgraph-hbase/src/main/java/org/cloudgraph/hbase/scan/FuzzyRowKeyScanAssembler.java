...
public class FuzzyRowKeyScanAssembler
	...
	private void assemblePredefinedFields() {
		List<PreDefinedKeyFieldConfig> preDefinedFields = this.graph
		...
		for (int i = 0; i < fieldCount; i++) {
			...
			byte[] tokenValue = this.keySupport
					.getPredefinedFieldValueStartBytes(this.rootType, hashing,
							preDefinedField);
	...
	private void assembleLiterals() {
		for (KeyFieldConfig fieldConfig : this.graph.getRowKeyFields()) {
			...
			if (fieldConfig instanceof PreDefinedKeyFieldConfig) {
				...
				this.keyBytes.put(paddedTokenValue);
				byte[] tokenMask = getPredefinedTokenMask(predefinedConfig,
						paddedTokenValue.length);
				this.infoBytes.put(tokenMask);
				...
				if (scanLiterals == null || scanLiterals.size() == 0) {
					...
					Arrays.fill(wildcard, (byte) Character.valueOf('*')
							.charValue());
					...
					FuzzyRowKeyLiteral fuzzyLiteral = (FuzzyRowKeyLiteral) scanLiterals
							.get(0);
					if (WildcardStringLiteral.class
							.isAssignableFrom(fuzzyLiteral.getClass())) {
						WildcardStringLiteral wc = (WildcardStringLiteral) fuzzyLiteral;
						String literal = wc.getContent();
						if (literal.length() != wc.getFieldConfig()
								.getMaxLength()) {
	...
	private byte[] getPredefinedTokenMask(
			PreDefinedKeyFieldConfig predefinedConfig,
	...
	public byte[] getFuzzyKeyBytes() {
		...
		System.arraycopy(this.keyBytes.array(), this.keyBytes.arrayOffset(),
				result, 0, this.keyBytes.position());
		return result;
	}
	...
	public byte[] getFuzzyInfoBytes() {
		...
		System.arraycopy(this.infoBytes.array(), this.infoBytes.arrayOffset(),
				result, 0, this.infoBytes.position());
		return result;
	}
	...
	public Filter getFilter() {
		Pair<byte[], byte[]> pair = new Pair<byte[], byte[]>(
				this.getFuzzyKeyBytes(), getFuzzyInfoBytes());

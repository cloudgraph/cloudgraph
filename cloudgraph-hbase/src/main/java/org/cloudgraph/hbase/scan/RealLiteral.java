...
public class RealLiteral extends ScanLiteral
	...
	public static final BigDecimal INCREMENT_DECIMAL = BigDecimal
	 ...
	 *         flavor) literal under the various optionally configurable
	 *         hashing, formatting and padding features.
	 */
	public byte[] getEqualsStartBytes() {
		byte[] startBytes = null;
		Object startValue = this.dataConverter.convert(property.getType(),
				this.literal);
	 ...
	 * relational operator under an HBase partial row-key scan for this real
	...
	public byte[] getLessThanStopBytes() {
		byte[] stopBytes = null;
		Object stopValue = this.dataConverter.convert(property.getType(),
				this.literal);
	...
	private String increment(Type type, Object value) {
		...
		switch (sourceDataType) {
			case Float :
				Float floatValue = this.dataConverter.toFloat(
						property.getType(), value);
				int intBits = Float.floatToRawIntBits(floatValue.floatValue());
				intBits++;
				Float floatResult = Float
						.valueOf(Float.intBitsToFloat(intBits));
				result = this.dataConverter.toString(type, floatResult);
				break;
			case Double :
				Double doubleValue = this.dataConverter.toDouble(
						property.getType(), value);
				long longBits = Double.doubleToRawLongBits(doubleValue
						.doubleValue());
				longBits++;
				Double doubleResult = Double.valueOf(Double
						.longBitsToDouble(longBits));
				result = this.dataConverter.toString(type, doubleResult);
				break;
			case Decimal :
				BigDecimal decimalValue = this.dataConverter.toDecimal(
						property.getType(), value);
				...
				longBits++;
				doubleResult = Double
						.valueOf(Double.longBitsToDouble(longBits));
				result = this.dataConverter.toString(type, doubleResult);
				break;
			default :
				throw new ScanException(
						"expected real (Float, Double, Decinal)datatype not, "
								+ sourceDataType.name());
		}
		return result;
	}

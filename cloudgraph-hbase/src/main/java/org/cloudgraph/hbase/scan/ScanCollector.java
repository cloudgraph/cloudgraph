...
public class ScanCollector implements ExprVisitor {
	...
	private void init() {
		if (this.partialKeyScans == null) {
			this.partialKeyScans = new ArrayList<PartialRowKey>(
					this.literals.size());
			this.fuzzyKeyScans = new ArrayList<FuzzyRowKey>(
					this.literals.size());
			this.completeKeys = new ArrayList<CompleteRowKey>(
					this.literals.size());
			for (Map<UserDefinedRowKeyFieldConfig, List<ScanLiteral>> existing : this.literals) {
				...
				if (scanLiterals.supportCompleteRowKey(this.graph)) {
					CompleteRowKeyAssembler assembler = new CompleteRowKeyAssembler(
							this.rootType);
					assembler.assemble(scanLiterals);
					this.completeKeys.add(assembler);
				} else if (scanLiterals.supportPartialRowKeyScan(this.graph)) {
					PartialRowKeyScanAssembler assembler = new PartialRowKeyScanAssembler(
							this.rootType);
					assembler.assemble(scanLiterals);
					this.partialKeyScans.add(assembler);
				} else {
					FuzzyRowKeyScanAssembler assembler = new FuzzyRowKeyScanAssembler(
							this.rootType);
					assembler.assemble(scanLiterals);
	...
	private void collect(RelationalBinaryExpr target, Expr source) {
		UserDefinedRowKeyFieldConfig fieldConfig = graph
				.getUserDefinedRowKeyField(target.getPropertyPath());
		if (fieldConfig == null) {
		...
		ScanLiteral scanLiteral = factory.createLiteral(target.getLiteral()
				.getValue(), property, (PlasmaType) graph.getRootType(), target
				.getOperator(), fieldConfig);
	...
	private void collect(ScanLiteral scanLiteral,
			UserDefinedRowKeyFieldConfig fieldConfig, Expr source) {
		if (source != null) {
			if (source instanceof LogicalBinaryExpr) {
				...
				throw new IllegalOperatorMappingException(
						"expected logical binary expression parent not, "
	...
	private void collect(UserDefinedRowKeyFieldConfig fieldConfig,
			LogicalBinaryExpr source, ScanLiteral scanLiteral) {
		if (this.literals.size() == 0) {
			...
			for (Map<UserDefinedRowKeyFieldConfig, List<ScanLiteral>> existingMap : literals) {
				if (source == null
						|| source.getOperator().getValue().ordinal() == LogicalOperatorValues.AND
								.ordinal()) {
					...
					if (list == null) {
						...
						RelationalOperatorValues existingOperator = existingLiteral
								.getRelationalOperator().getValue();
						switch (scanLiteral.getRelationalOperator().getValue()) {
							case GREATER_THAN :
							case GREATER_THAN_EQUALS :
								if (existingOperator.ordinal() != RelationalOperatorValues.LESS_THAN
										.ordinal()
										&& existingOperator.ordinal() != RelationalOperatorValues.LESS_THAN_EQUALS
												.ordinal())
									throw new ImbalancedOperatorMappingException(
											scanLiteral.getRelationalOperator()
													.getValue(),
											LogicalOperatorValues.AND,
											existingOperator, fieldConfig);
								list.add(scanLiteral);
								break;
							case LESS_THAN :
						...
						throw new IllegalOperatorMappingException(
								"logical operator '"
										+ LogicalOperatorValues.AND
										+ "' mapped more than 2 times "
										+ "to row key field property, "
										+ fieldConfig.getEndpointProperty()
												.getContainingType().toString()
										+ "."
										+ fieldConfig.getEndpointProperty()
												.getName());
					}
				} else if (source.getOperator().getValue().ordinal() == LogicalOperatorValues.OR
						.ordinal()) {
					List<ScanLiteral> list = existingMap.get(fieldConfig);
					if (list == null) {
						if (foundField)
							throw new IllegalStateException(
									"expected for key field mapped to scans "
			...
			if (foundField) {
				...
				Map<UserDefinedRowKeyFieldConfig, List<ScanLiteral>> next = newMap(
						literals.get(0), fieldConfig, scanLiteral);
				literals.add(next);
			}
		}
	}

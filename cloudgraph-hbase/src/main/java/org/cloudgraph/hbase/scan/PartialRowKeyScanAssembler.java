...
public class PartialRowKeyScanAssembler
	...
	private static Log log = LogFactory
			.getLog(PartialRowKeyScanAssembler.class);
	...
	/**
	 * Constructor which enables the use of data object UUID as a pre-defined
	 * row key field. Only applicable for graph predicate "slice" queries.
	 * 
	 * @param rootType
	 *            the root type
	 * @param rootUUID
	...
	/**
	 * Assemble row key scan information based on the given scan literals as
	 * well as pre-defined row-key fields such as the data graph root type or
	 * URI.
	 * 
	 * @param literalList
	 *            the scan literals
	...
	private void assemblePredefinedFields() {
		...
		for (PreDefinedKeyFieldConfig field : this.graph
		...
		for (int i = 0; i < fieldCount; i++) {
			...
			byte[] paddedStopValue = getStopBytes(preDefinedField,
					i >= (fieldCount - 1));
	...
	private void assembleLiterals() {
		...
		for (KeyFieldConfig fieldConfig : this.graph.getRowKeyFields()) {
			if (fieldConfig instanceof PreDefinedKeyFieldConfig) {
				...
				List<ScanLiteral> scanLiterals = this.scanLiterals
						.getLiterals(userFieldConfig);
				...
				for (ScanLiteral scanLiteral : scanLiterals) {
					...
					if (startBytes.length > 0) {
						if (this.startRowFieldCount > 0) {
							this.startKey.put(graph
									.getRowKeyFieldDelimiterBytes());
						}
	...
	private byte[] getStartBytes(PreDefinedKeyFieldConfig preDefinedField) {
		...
		switch (preDefinedField.getName()) {
			case UUID :
				...
				break;
			default :
				startValue = this.keySupport.getPredefinedFieldValueStartBytes(
						this.rootType, hashing, preDefinedField);
				break;
		}
		byte[] paddedStartValue = null;
		if (preDefinedField.isHash()) {
			paddedStartValue = this.padding.pad(startValue,
					preDefinedField.getMaxLength(), DataFlavor.integral);
		} else {
			paddedStartValue = this.padding.pad(startValue,
	...
	private byte[] getStopBytes(PreDefinedKeyFieldConfig preDefinedField,
			boolean lastField) {
		byte[] stopValue = null;
		switch (preDefinedField.getName()) {
			case UUID :
			...
			default :
				if (!lastField)
					stopValue = this.keySupport
							.getPredefinedFieldValueStartBytes(this.rootType,
									hashing, preDefinedField);
				else
					stopValue = this.keySupport
							.getPredefinedFieldValueStopBytes(this.rootType,
									hashing, preDefinedField);
				break;
		}
	...
	public byte[] getStartKey() {
		if (this.startKey == null)
			throw new IllegalStateException(
					"row keys not assembled - first call assemble(...)");
		...
		System.arraycopy(this.startKey.array(), this.startKey.arrayOffset(),
				result, 0, this.startKey.position());
		return result;
	}
	...
	public byte[] getStopKey() {
		...
		System.arraycopy(this.stopKey.array(), this.stopKey.arrayOffset(),
				result, 0, this.stopKey.position());
		return result;
	}
}

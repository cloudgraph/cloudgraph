...
public class ScanLiteralAssembler extends DefaultQueryVisitor {
	...
	/**
	 * Assemble the set of data "flavor" and data type specific scan literals
	 * used to construct composite partial row (start/stop) key pair.
	 * 
	 * @param where
	...
	public void start(Literal literal) {
		...
		if (fieldConfig != null) {
			...
			if (this.contextRelationalOperator != null) {
				scanLiteral = this.scanLiteralFactory.createLiteral(content,
						property, this.rootType,
				...
				scanLiteral = this.scanLiteralFactory.createLiteral(content,
						property, this.rootType, this.contextWildcardOperator,
						fieldConfig);
				this.fuzzyKeyScanLiterals.addLiteral(scanLiteral);
			} else
				throw new GraphServiceException(
						"expected relational or wildcard operator for query path '"
	...
	public void start(NullLiteral nullLiteral) {
		throw new GraphServiceException(
				"null literals for row scans not yet supported");
	}

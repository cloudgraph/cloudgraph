...
public abstract class SocialGraphModelTest extends HBaseTestCase {
	...
	protected GraphInfo createGraph() throws IOException {
		...
		String name = USERNAME_BASE
				+ String.valueOf(System.currentTimeMillis()) + "test.com";
		...
		info.friendship = (Friendship) info.actor
				.createTargetEdge(Friendship.class);
		...
		info.atlanticWeather
				.setDescription("a topic related to weather specific to the Atlantic");
	...
	protected GraphInfo createSimpleGraph() throws IOException {
		...
		String name = USERNAME_BASE
				+ String.valueOf(System.currentTimeMillis()) + "_test.com";
		info.actor = createRootActor(name);
		info.actor.setName(name);
		info.actor.setDescription("I'm a guy who likes storms...");
	...
	protected QActor createTopicScanQuery(String name, Topic topic) {
		...
		root.select(root.wildcard()).select(root.sourceEdge().wildcard())
				...
				.select(root.targetEdge().source().wildcard())
				.select(root.targetEdge().target().wildcard())
				.select(root.blog().wildcard()).select(root.photo().wildcard());
	...
	protected QActor createTopicScanQuery(String name1, String name2,
			String name3, Topic topic1, Topic topic2) {
		QActor root = QActor.newQuery();
		root.select(root.wildcard()).select(root.sourceEdge().wildcard())
				.select(root.sourceEdge().source().wildcard())
				.select(root.sourceEdge().target().wildcard())
	...
	protected QActor createTopicWildcardScanQuery(String name) {
		...
		root.where(root
				.name()
				.eq(name)
				.and(root.blog().topic().name().like("*ics")
						.and(root.blog().topic().name().like("*lit*"))));
	...
	protected QActor createBlogPredicateQuery(String actorName, String blogName) {
		...
		actor.select(actor.wildcard())
				...
				.select(actor.blog(blogPredicate).topic().child().child()
						.wildcard()).select(actor.photo().wildcard());
	...
	protected QActor createPhotoPredicateQuery(String actorName,
			String photoName) {
		QActor actor = QActor.newQuery();

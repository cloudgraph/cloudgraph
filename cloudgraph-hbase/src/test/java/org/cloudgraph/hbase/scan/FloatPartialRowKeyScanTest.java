...
public class FloatPartialRowKeyScanTest extends DataTypeGraphModelTest {
	private static Log log = LogFactory
			.getLog(FloatPartialRowKeyScanTest.class);
	...
	public void testBetween() throws IOException {
		...
		Node[] fetched = this.fetchGraphsBetween(rootId, root1.getFloatField(),
				root3.getFloatField());
	...
	protected Node[] fetchGraphsBetween(long rootId, Float min, Float max) {
		...
		root.where(root.rootId().eq(rootId)
				.and(root.floatField().between(min, max)));
	...
	protected FloatNode createGraph(long rootId, long id, Date now,
			String namePrefix) {

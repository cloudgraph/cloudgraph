...
public class DateTimePartialRowKeyScanTest extends DataTypeGraphModelTest {
	private static Log log = LogFactory
			.getLog(DateTimePartialRowKeyScanTest.class);
	private long WAIT_TIME = 10;
	private String USERNAME = "datetime_test";

	public static Test suite() {
		return PlasmaTestSetup
				.newTestSetup(DateTimePartialRowKeyScanTest.class);
	}
	...
	public void testEqual() throws IOException {
		...
		// fetch a slice
		Node fetched = this.fetchSingleGraph(rootId, id2, root2.getChild(3)
				.getName(), root2.getDateTimeField());
	...
	protected Node fetchSingleGraph(long rootId, long id, String name,
			Object date) {
	...
	protected Node[] fetchGraphsBetween(long rootId, long min, long max,
		...
		root.where(root.rootId().eq(rootId)
				.and(root.dateTimeField().between(minDate, maxDate)));
	...
	protected Node[] fetchGraphsInclusive(long rootId, long min, long max,
		...
		root.where(root
				.rootId()
				.eq(rootId)
				.and(root.dateTimeField().ge(minDate)
	...
	protected Node[] fetchGraphsExclusive(long min, long max, Object minDate,
			Object maxDate) {
		QDateTimeNode root = createSelect();

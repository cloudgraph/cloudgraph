...
public class StringPartialRowKeyScanTest extends StringScanTest {
	private static Log log = LogFactory
			.getLog(StringPartialRowKeyScanTest.class);
	...
	public void testPartialKeyWildcard() throws IOException {
		...
		assertTrue("expected 3 results not " + fetched.length,
				fetched.length == 3);
	...
	public void testRange() throws IOException {
		...
		debugGraph(result[0]);
		assertTrue("AAA_0_0".equals(((Node) result[0].getRootObject())
				.getStringField()));
		assertTrue("EEE_0_0".equals(((Node) result[4].getRootObject())
				.getStringField()));
		...
		assertTrue("AAA_0_0".equals(((Node) result[0].getRootObject())
				.getStringField()));
		assertTrue("CCC_0_0".equals(((Node) result[2].getRootObject())
				.getStringField()));
		...
		assertTrue("CCC_0_0".equals(((Node) result[0].getRootObject())
				.getStringField()));
		assertTrue("DDD_0_0".equals(((Node) result[1].getRootObject())
				.getStringField()));
	}
	...
	protected Node fetchSingleGraph(long id, String name) {
		...
		assertTrue(result != null);
		// FIXME: failing
		assertTrue("expected 1 results not " + result.length,
				result.length == 1);
	...
	protected Node[] fetchGraphsBetween(long rootId, String minName,
			String maxName) {
		QStringNode root = createSelect();
		root.where(root.rootId().eq(rootId)
	...
	protected Node[] fetchGraphsInclusive(long rootId, String minName,
			String maxName) {
		QStringNode root = createSelect();
	...
	protected Node[] fetchGraphsExclusive(long rootId, String minName,
			String maxName) {
		QStringNode root = createSelect();

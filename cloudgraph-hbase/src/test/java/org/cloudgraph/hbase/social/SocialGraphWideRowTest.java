...
/**
 * Tests standard operations on a graph, where the graph is stored as a single "wide" row
 * as opposed to being distributed across HBase tables. See the associated *cloudgraph-config.xml
 * which maps only the social graph {@link Actor} root to a table, the remaining graph modes
 * being "unbound" and therefore part of the wide row.    
 *  
...
{    
    ...
    public void testUpdateDataProperties() throws IOException       
    {
    	...
    	for (Photo photo : fetchedActor.getPhoto()) {
    		...
    		photo.setDescription(photo.getDescription() + " - updated photo description " + String.valueOf(i));
    		i++;
    	}
    ...
    public void testUpdateAddChildTopic() throws IOException       
    {
    	...
    	this.service.commit(fetchedActor.getDataGraph(), 
    			"test1");
    ...
    public void testUpdateAddChildTopicHierarchy() throws IOException       
    {
    	...
    	Topic topic = (Topic)fetchedActor.get("blog[name='Hurricane Sandy']/topic[name='Politics']");
   	    ...
   	    grandChild2.setDescription("The new Jersey Governor and his 'Beach Gate' scandal where he forbid all NJ citizens from the beach, then went there w/his family");

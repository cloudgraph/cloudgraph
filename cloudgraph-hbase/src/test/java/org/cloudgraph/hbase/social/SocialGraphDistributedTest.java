...
public class SocialGraphDistributedTest extends SocialGraphModelTest {
	private static Log log = LogFactory
			.getLog(SocialGraphDistributedTest.class);
	...
	public void testInsert() throws IOException {
		...
		Actor fetchedFollower = (Actor) fetchedActor.getTargetEdge(0)
				.getSource();
		...
		Blog fetchedBlog = (Blog) fetchedActor.get("blog[@name='"
		...
		Actor fetchedFollowerRoot = fetchGraph(createFollowerGraphQuery(info.follower
				.getName()));
		...
		Actor fetchedActorSliceRoot = fetchGraph(createBlogPredicateQuery(
				info.actor.getName(), info.electionBlog.getName()));
		xml = this.serializeGraph(fetchedActorSliceRoot.getDataGraph());
		log.debug(xml);
		assertTrue(fetchedActorSliceRoot.getBlogCount() == 1);
		fetchedBlog = (Blog) fetchedActorSliceRoot.get("blog[@name='"
				+ info.electionBlog.getName() + "']");
		assertTrue(fetchedBlog != null);
		assertTrue(fetchedActor.getTargetEdgeCount() == 1);
		...
		fetchedActorSliceRoot = fetchGraph(createPhotoPredicateQuery(
				info.actor.getName(), info.photo2.getName()));
		xml = this.serializeGraph(fetchedActorSliceRoot.getDataGraph());
		log.debug(xml);
		assertTrue(fetchedActorSliceRoot.getPhotoCount() == 1);
		Photo fetchedPhoto = (Photo) fetchedActorSliceRoot.get("photo[@name='"
				+ info.photo2.getName() + "']");
	...
	public void testUpdate() throws IOException {
		...
		log.debug("fetching follower graph");
		Actor fetchedFollowerRoot = fetchGraph(createGraphQuery(info.follower
				.getName()));
		xml = this.serializeGraph(fetchedFollowerRoot.getDataGraph());
		log.debug(xml);

		assertTrue(fetchedFollowerRoot.getBlogCount() == 1);
		Blog fetchedBlog = (Blog) fetchedFollowerRoot.get("blog[@name='"
				+ blog.getName() + "']");
		assertTrue(fetchedBlog != null);
		assertTrue(fetchedBlog.getTopicCount() == 1);
		Topic fetchedTopic = fetchedBlog.getTopic(0);
		...
		fetchedFollowerRoot = fetchGraph(createGraphQuery(info.follower
				.getName()));
		xml = this.serializeGraph(fetchedFollowerRoot.getDataGraph());
		log.debug(xml);
		...
		fetchedFollowerRoot = findGraph(createGraphQuery(info.follower
				.getName()));
		assertTrue(fetchedFollowerRoot == null);
		...
		Actor fetchedFollower = (Actor) fetchedRoot.getTargetEdge(0)
				.getSource();
	...
	public void testTopicInsertAndLink() throws IOException {
		...
		String name = USERNAME_BASE
				+ String.valueOf(System.currentTimeMillis()) + "_example.com";
		...
		physicsBlog
				.setDescription("Magnetization parameters and temperature...");
	...
	public void testBlogAdd() throws IOException {
		...
		String name = USERNAME_BASE
				+ String.valueOf(System.currentTimeMillis()) + "@example.com";
		Actor actor = createRootActor(name);
		actor.setName(name);
		...
		metamorphicRocksBlog
				.setDescription("Metamorphic rocks are cool because...");
		...
		sedementaryRocksBlog
				.setDescription("Sedementary rocks are cool because...");
		...
		for (Blog blog : fetchedActor.getBlog()) {
			for (Topic topic : blog.getTopic()) {
				...
				if (topic.getChildCount() == 0)
					if (!fetchedActor.getDataGraph().getChangeSummary()
							.isDeleted(topic))
						topic.delete();
			}
		}

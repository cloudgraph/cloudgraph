...
public class GraphQuery implements QueryDispatcher {
	...
	public PlasmaDataGraph[] find(Query query, int requestMax,
		...
		List<List<PropertyPair>> queryResults = findResults(query, collector,
				type, con);
		...
		switch (fetchType) {
			case PARALLEL :
				...
				assembler = new ParallelGraphAssembler(type, collector,
						snapshotDate, config, con);
				break;
			case SERIAL :
			default :
				assembler = new GraphAssembler(type, collector, snapshotDate,
						con);
				break;
		}
		...
		try {
			if (!query.getSelectClause().hasDistinctProperties())
				...
				results = trimResults(queryResults, requestMax, assembler,
	...
	/**
	 * Iterate over the given results collection up to the given limit,
	 * assembling a data-object graph for each result. In the simplest case,
	 * only a single root data-object is assembled.
	 * 
	 * @param collection
	 *            - the results collection
	 * @param requestMax
	 *            - the requested max or limit of items required in the results.
	 *            If results exceed the given maximum, then they are truncated.
	 *            If no maximum value is given (e.g. -1) then a default max is
	 *            enforced.
	 * @param assembler
	 *            - the value object assembler
	 * @throws SQLException
	 * @throws MaxResultsExceededException
	 *             when no request maximum is given and the default max is
	 *             exceeded.
	 */
	private PlasmaDataGraph[] assembleResults(
			List<List<PropertyPair>> collection, int requestMax,
		...
		for (int i = 1; iter.hasNext(); i++) {
			...
			if (requestMax <= 0) {
				if (i > QueryConstants.MAX_RESULTS) // note: this is cheezy but
													...
													// size is expensive
					throw new MaxResultsExceededException(i,
							QueryConstants.MAX_RESULTS);
			} else if (i > requestMax) {
				if (log.isDebugEnabled()) {
					log.debug("truncating results at "
							+ String.valueOf(requestMax));
				}
				break;
			}
	 ...
	 * after assembly based on any "distinct" property(ies) in the results
	 * object graph. Results must be pruned/trimmed after assembly because the
	 * assembly invokes POM traversals/queries where the "distinct"
	 * propertiy(ies) are realized. For each result, a hash key is assembled to
	 * determine it's uniqueness based on a search of the assembled value-object
	 * graph for "distinct" properties as defined in the given Query select
	 * clause.
	 * 
	 * @param collection
	 *            - the results collection
	...
	private PlasmaDataGraph[] trimResults(List<List<PropertyPair>> collection,
			int requestMax, LangStoreGraphAssembler assembler, Select select,
			Type type) throws SQLException {
		DataObjectHashKeyAssembler hashKeyAssembler = new DataObjectHashKeyAssembler(
				select, type);
		// FIXME rely on non-ordered map to retain initial ordering and get
		// rid of list collection.
		Map<String, PlasmaDataObject> distinctMap = new HashMap<String, PlasmaDataObject>(
				20);
		List<PlasmaDataObject> distinctList = new ArrayList<PlasmaDataObject>(
				20);
		Iterator<List<PropertyPair>> iter = collection.iterator();
		...
		for (i = 1; iter.hasNext(); i++) {
			...
			assembler.clear();
			String key = hashKeyAssembler
					.getHashKey((PlasmaDataObject) dataGraph.getRootObject());
			if (distinctMap.get(key) == null) {
				if (requestMax <= 0) {
					if (i > QueryConstants.MAX_RESULTS) // note: this is cheezy
														...
														// collection size is
														// expensive
						throw new MaxResultsExceededException(
								distinctList.size(), QueryConstants.MAX_RESULTS);
				} else if (i > requestMax) {
					if (log.isDebugEnabled()) {
						log.debug("truncating results at "
								+ String.valueOf(requestMax));
				...
				distinctMap.put(key,
						(PlasmaDataObject) dataGraph.getRootObject());
				distinctList.add((PlasmaDataObject) dataGraph.getRootObject());
			}
		}
		PlasmaDataGraph[] trimmedResults = new PlasmaDataGraph[distinctList
				.size()];
		for (i = 0; i < trimmedResults.length; i++)
			trimmedResults[i] = (PlasmaDataGraph) distinctList.get(i)
					.getDataGraph();

		if (log.isDebugEnabled()) {
			log.debug("assembled " + String.valueOf(trimmedResults.length)
					+ " results out of " + String.valueOf(i));
		}
	...
	private int countResults(Connection con, Query query, PlasmaType type) {
		...
		while (it.hasNext()) {
			...
			if (count > 0)
				sqlQuery.append(", ");
			sqlQuery.append(this.statementUtil
					.getQualifiedPhysicalName(aliasType));
			sqlQuery.append(" ");
		...
		if (query.getStartRange() != null && query.getEndRange() != null)
			log.warn("query range (start: " + query.getStartRange() + ", end: "
	...
	private List<List<PropertyPair>> findResults(Query query,
			SelectionCollector collector, PlasmaType type, Connection con) {
		Object[] params = new Object[0];
		...
		switch (vendor) {
			case ORACLE :
				if (query.getStartRange() != null
						&& query.getEndRange() != null) {
		...
		try {
			...
			while (rs.next()) {
				...
				for (i = 1; i <= numcols; i++) {
					...
					PlasmaProperty prop = (PlasmaProperty) type
							.getProperty(columnName);
					PlasmaProperty valueProp = prop;
					while (!valueProp.getType().isDataType()) {
						valueProp = this.statementUtil
								.getOppositePriKeyProperty(valueProp);
					}

					Object value = converter.fromJDBCDataType(rs, i,
							columnType, valueProp);
					if (value != null) {
			...
			if (log.isDebugEnabled())
				log.debug("read " + numresults + " results ("
						+ String.valueOf(after - before) + ")");
		} catch (Throwable t) {
			StringBuffer buf = this.generateErrorDetail(t, sqlQuery.toString(),
					filterAssembler);

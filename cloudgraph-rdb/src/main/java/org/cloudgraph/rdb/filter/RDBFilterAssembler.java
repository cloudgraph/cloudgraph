...
public class RDBFilterAssembler extends SQLQueryFilterAssembler
	...
	public RDBFilterAssembler(Where where, Type contextType, AliasMap aliasMap) {
		...
		if (where.getTextContent() == null && where.getFilterId() == null) {
			...
			if (where.getParameters().size() > 0)
				throw new DataAccessException(
						"parameters allowed only for 'free-text' Where clause");
			...
			if (where.getTextContent() != null) {
				...
				throw new QueryException(
						"expected free-text content or filter id");
		}
	}
	...
	public void start(Expression expression) {
		...
		for (int i = 0; i < expression.getTerms().size(); i++) {
			SubqueryOperator subqueryOper = expression.getTerms().get(i)
					.getSubqueryOperator();
			if (subqueryOper != null) {
				// log.info("found subquery expression");
				Property property = expression.getTerms().get(i - 1)
						.getProperty();
				Query query = (Query) expression.getTerms().get(i + 1)
						.getQuery();
	...
	protected void assembleSubquery(Property property, SubqueryOperator oper,
		...
		if (property.getPath() != null)
			throw new QueryException("properties with paths ("
					+ property.getName() + ") not allowed as subquery target");

		commonj.sdo.Property endpointProperty = contextType
		...
		switch (oper.getValue()) {
			case IN :
				...
				filter.append(DATA_ACCESS_CLASS_MEMBER_PREFIX
	 ...
	 * 'AND' expressions based on key relationships until the property endpoint
	 * is reached. Superclass handlers deal with other query nodes such as
	...
	public void start(Property property) {
		...
		if (path != null) {
			...
			for (int i = 0; i < path.getPathNodes().size(); i++) {
				...
				if (!prop.isMany()) {
					filter.append(prevTargetAlias + "."
							+ prop.getPhysicalName());
					filter.append(" = ");
					PlasmaProperty priKeyProp = (PlasmaProperty) targetType
							.findProperty(KeyType.primary);
					filter.append(targetAlias + "."
							+ priKeyProp.getPhysicalName());
				} else {
					PlasmaProperty opposite = (PlasmaProperty) prop
							.getOpposite();
					if (opposite.isMany())
						throw new DataAccessException(
								"expected singular opposite for property, "
										+ prop.getContainingType().getURI()
										+ "#"
										+ prop.getContainingType().getName()
										+ "." + prop.getName());
					filter.append(targetAlias + "."
							+ opposite.getPhysicalName());
					filter.append(" = ");
					PlasmaProperty priKeyProp = (PlasmaProperty) prevTargetType
							.findProperty(KeyType.primary);
					filter.append(prevTargetAlias + "."
							+ priKeyProp.getPhysicalName());
				}

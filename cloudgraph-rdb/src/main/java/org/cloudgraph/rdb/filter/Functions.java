...
public class Functions {
	...
	public static String wrap(PlasmaProperty endpointProp,
			List<Function> functions, String targetAlias) {
	...
	private static String beginFunctionOracle(Function func,
			PlasmaProperty endpointProp, String propertyArg) {
		StringBuilder buf = new StringBuilder();
	...
	private static String beginFunctionMySql(Function func,
			PlasmaProperty endpointProp, String propertyArg) {
		StringBuilder buf = new StringBuilder();
		switch (func.getName()) {
			case MIN :
	...
	/**
	 * Performs pre checks on the given function ensuring the correct datatype
	 * and number of arguments, etc..
	 * 
	 * @param func
	 *            the function
	 * @param endpointProp
	...
	private static void validateFunction(Function func,
			PlasmaProperty endpointProp, DataFlavor flavor) {
		...
		switch (func.getName()) {
			...
			case AVG :
			case SUM :
				throw new DataAccessException("aggregate function '"
						+ func.getName()
			...
			case ROUND :
				if (!isNumber(flavor))
					throw new DataAccessException("function '" + func.getName()
							+ "' not applicable for non numeric property, "
			...
			case LOWER_CASE :
				if (!isString(flavor))
					throw new DataAccessException("function '" + func.getName()
							+ "' not applicable for non-string property, "
							+ endpointProp.toString());
				break;
			case YEAR_FROM_DATE :
			case MONTH_FROM_DATE :
			case DAY_FROM_DATE :
				if (!isString(flavor))
					throw new DataAccessException("function '" + func.getName()
							+ "' not applicable for non-temporal property, "
							+ endpointProp.toString());
				break;
		}
	}

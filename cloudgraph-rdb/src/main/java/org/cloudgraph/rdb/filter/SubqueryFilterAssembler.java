...
public class SubqueryFilterAssembler extends SQLQueryFilterAssembler {
	...
	public SubqueryFilterAssembler(String alias, Query query, List params,
			Type contextType) {
	...
	public void start(Select select) {
		if (select.getProperties().size() > 1)
			throw new QueryException(
					"multiple properties on subqueries not supported");
		...
		if (property.getPath() != null)
			throw new QueryException(
					"property paths not supported on subquery 'select'");

		commonj.sdo.Property prop = contextType
				.getProperty(((Property) property).getName());
		if (!prop.getType().isDataType())
			throw new QueryException("reference properties ("
					+ contextType.getName() + "." + prop.getName()
					+ ") not supported on subquery 'select'");
		if (prop.isMany())
		...
		filter.append("from org.plasma.sdo.das.pom." + contextType.getName()
				+ " " + alias);
	...
	public void start(Property property) {
		...
		if (path != null) {
			for (int i = 0; i < path.getPathNodes().size(); i++) {
				...
				commonj.sdo.Property pdef = targetType
						.getProperty(((PathElement) pathElem).getValue());
				...
				if (!pdef.isMany()) {
					...
					filter.append(DATA_ACCESS_CLASS_MEMBER_PREFIX
							+ pdef.getName()
							+ DATA_ACCESS_CLASS_MEMBER_MULTI_VALUED_SUFFIX
		...
		// property.getName());
		commonj.sdo.Property endpoint = targetType.getProperty(property
				.getName());

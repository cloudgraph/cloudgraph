 ...
 * @since 0.6.2
 */
public class ParallelSubgraphTask extends AssemblerSupport
		implements
			SubgraphTask {
	...
	public ParallelSubgraphTask(PlasmaType subrootType,
			PlasmaDataObject source, SelectionCollector selection,
			StatementFactory statementFactory,
			StatementExecutor statementExecutor, PlasmaProperty sourceProperty,
			List<PropertyPair> childKeyPairs, int level, int sequence,
		...
		if (log.isDebugEnabled())
			log.debug(String.valueOf(level) + ":process: "
	...
	public void assemble() {
		assemble(this.subrootType, this.source, this.sourceProperty,
				this.subrootChildKeyPairs, this.level);
	}
	...
	private void assemble(PlasmaType targetType, PlasmaDataObject source,
		...
		if (log.isDebugEnabled())
			log.debug(String.valueOf(level) + ":assemble: "
					+ source.getType().getName() + "."
					+ sourceProperty.getName() + "->" + targetType.getName()
					+ ": " + props);

		List<List<PropertyPair>> result = this.getPredicateResult(targetType,
				sourceProperty, props, childKeyPairs);

		if (log.isDebugEnabled())
			log.debug(String.valueOf(level) + ":results: " + result.size());

		Map<PlasmaDataObject, List<PropertyPair>> resultMap = this.sharedAssembler
				.collectResults(targetType, source, sourceProperty, result);
		...
		while (iter.hasNext()) {
			...
			for (PropertyPair pair : row) {
				if (pair.getProp().isMany()
						|| pair.getProp().getType().isDataType())
				...
				if (!pair.isQueryProperty())
					continue; // property is a key or other property not
								...
								// traverse it
				List<PropertyPair> nextKeyPairs = this.getNextKeyPairs(target,
						pair, level);

				if (log.isDebugEnabled())
					log.debug(String.valueOf(level) + ":traverse: ("
							+ pair.getProp().isMany() + ") "
							+ pair.getProp().toString() + ":"
							+ String.valueOf(pair.getValue()));

				Traversal trav = new Traversal((PlasmaType) pair.getProp()
						.getType(), target, pair.getProp(), nextKeyPairs,
						level + 1);
				traversals.add(trav);
			}
			...
			for (Property p : props) {
				...
				List<PropertyPair> childKeyProps = this.getChildKeyProps(
						target, targetType, prop);
				if (log.isDebugEnabled())
					log.debug(String.valueOf(level) + ":traverse: ("
							+ prop.isMany() + ") " + prop.toString() + " - "
			...
			for (int i = 0; i < available; i++) {
				...
				SubgraphTask task = new ParallelSubgraphTask(
						trav.getSubrootType(), trav.getSource(),
						this.collector, this.getStatementFactory(),
						this.getStatementExecutor(), trav.getSourceProperty(),
						trav.getChildKeyPairs(), trav.getLevel(), i,
			...
			for (int i = available; i < traversals.size(); i++) {
				Traversal trav = traversals.get(i);
				assemble(trav.getSubrootType(), trav.getSource(),

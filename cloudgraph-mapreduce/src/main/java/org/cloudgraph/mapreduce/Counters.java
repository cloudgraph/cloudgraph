...
public class Counters {
	...
	/**
	 * MapReduce counter which stores the number of data graphs not recognized
	 * by the current recognizer. Will remain zero if no recognizer is required
	...
	/**
	 * MapReduce counter which stores the total time in milliseconds taken for
	 * graph XMl unmarshalling. Note: this time counter is summed across all
	 * tasks on all hosts so could exceed the total time taken for the job, as
	 * the tasks of course run in parallel.
	 */
	 ...
	 * on all hosts so could exceed the total time taken for the job, as the
	 * tasks of course run in parallel.
	 */

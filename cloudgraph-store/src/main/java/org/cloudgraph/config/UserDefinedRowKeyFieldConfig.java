...
public class UserDefinedRowKeyFieldConfig extends KeyFieldConfig {
	private static Log log = LogFactory
			.getLog(UserDefinedRowKeyFieldConfig.class);
	...
	/**
	 * 
	 * Note; not using SDO XPath here as this operates on data not metadata and
	 * at this point we have only metadata to work with. This seems to surface
	 * the need for XPath processing for SDO types.
	 * 
	 * @param xpath
	 */
	private void construct(String xpath) {
		...
		for (int i = 0; i < tokens.length; i++) {
			...
			PlasmaProperty prop = (PlasmaProperty) contextType
					.getProperty(token);
			if (!prop.getType().isDataType()) {
				...
				if (i == tokens.length - 1)
					throw new CloudGraphConfigurationException(
							"expected xpath '"
									+ xpath
									+ "' termination with a data property not, "
									+ prop);
			} else
		...
		DataType dataType = DataType.valueOf(this.endpointProperty.getType()
				.getName());
		switch (flavor) {
			case string :
				this.maxLength = getStringTypeMaxLength(this.endpointProperty,
						dataType, flavor);
				break;
			case integral :
				this.maxLength = getIntegralTypeMaxLength(
						this.endpointProperty, dataType, flavor);
				break;
			case real :
				this.maxLength = getRealTypeMaxLength(this.endpointProperty,
						dataType, flavor);
				break;
			case temporal :
				this.maxLength = getTemporalTypeMaxLength(
						this.endpointProperty, dataType, flavor);
				break;
			case other :
	...
	private int getRealTypeMaxLength(PlasmaProperty prop, DataType dataType,
			DataFlavor flavor) {
		...
		if (constraint.getTotalDigits() == null
				|| constraint.getTotalDigits().length() == 0)
			throw new MissingRequiredConstraintException(
	...
	private int getStringTypeMaxLength(PlasmaProperty prop, DataType dataType,
			DataFlavor flavor) {
		if (prop.getValueConstraint() == null)
			throw new MissingRequiredConstraintException(
		...
		if (constraint.getMaxLength() == null
				|| constraint.getMaxLength().length() == 0)
			throw new MissingRequiredConstraintException(
	...
	private int getIntegralTypeMaxLength(PlasmaProperty prop,
		...
		if (prop.getValueConstraint() != null
				&& prop.getValueConstraint().getTotalDigits() != null
				&& prop.getValueConstraint().getTotalDigits().length() > 0) {
			result = Integer.parseInt(prop.getValueConstraint()
					.getTotalDigits());
			if (result == 0) {
		...
		switch (dataType) {
			case Short :
				log.warn("expected value contraint total-digits for integral property, "
						...
						+ " - defaulting to max size for datatype, "
						+ dataType
						+ ".");
		...
		if (result == null)
			throw new UnresolvedPathExpressionException(
					"the configured XPath expression '"
							...
							+ "'"
							+ " for graph root type '"
							+ dataGraph.getRootType().getName()
							+ "'"
							+ " within table '"
							+ dataGraph.getTable().getName()
							+ "'"

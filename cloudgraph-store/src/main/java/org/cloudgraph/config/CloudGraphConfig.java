...
public class CloudGraphConfig implements Config {
	...
	private CloudGraphConfig() {
		...
		try {
			...
			InputStream propertiesStream = CloudGraphConfiguration.class
					.getResourceAsStream(DEFAULT_PROPERTIES_FILE_NAME);
			if (propertiesStream == null)
				propertiesStream = CloudGraphConfig.class.getClassLoader()
						.getResourceAsStream(DEFAULT_PROPERTIES_FILE_NAME);
			if (propertiesStream != null) {
				configProperties.addConfiguration(new PropertiesConfiguration(
	...
	private void mapTable(TableConfig tableConfig) {
		if (this.tableNameToTableMap.get(tableConfig.getQualifiedName()) != null)
			throw new CloudGraphConfigurationException(
					"a table definition already exists for qualified name '"
							+ tableConfig.getQualifiedName() + "'");
		this.tableNameToTableMap.put(tableConfig.getQualifiedName(),
				tableConfig);
		for (DataGraph graph : tableConfig.getTable().getDataGraphs()) {
			DataGraphConfig dataGraphConfig = new DataGraphConfig(graph,
					tableConfig);
	...
	private void unmapTable(TableConfig tableConfig) {
		if (this.tableNameToTableMap.get(tableConfig.getQualifiedName()) == null)
			throw new CloudGraphConfigurationException(
					"table definition does not exist exists for qualified name '"
							+ tableConfig.getQualifiedName() + "'");
	...
	private void mapDataGraph(DataGraphConfig dataGraphConfig) {
		...
		QName qname = new QName(dataGraphConfig.getGraph().getUri(),
				dataGraphConfig.getGraph().getType());
		PlasmaType configuredType = (PlasmaType) PlasmaTypeHelper.INSTANCE
				.getType(qname.getNamespaceURI(), qname.getLocalPart());
		...
		if (graphURIToTableMap.get(qname) != null)
			throw new CloudGraphConfigurationException(
					"a data graph definition already exists within table '"
							+ dataGraphConfig.getTable().getTable().getName()
							+ "' for type (uri/name), "
							+ dataGraphConfig.getGraph().getUri() + "#"
							+ dataGraphConfig.getGraph().getType());
		 ...
		 * type.getQualifiedName(); if (graphURIToTableMap.get(qname) != null)
		 * throw new CloudGraphConfigurationException(
		 * "a data graph definition already exists within table '" +
		 * table.getName() + "' for type (uri/name), " + graph.getUri() + "#" +
		 * graph.getType()); graphURIToTableMap.put(qname, tableConfig);
		 * graphURIToGraphMap.put(qname, dataGraphConfig); }
		 */
	}
	...
	private void unmapDataGraph(DataGraphConfig dataGraphConfig) {
		...
		if (graphURIToTableMap.get(qname) == null)
			throw new CloudGraphConfigurationException(
					"no data graph definition already exists within table '"
							+ dataGraphConfig.getTable().getTable().getName()
							+ "' for type (uri/name), "
							+ dataGraphConfig.getGraph().getUri() + "#"
							+ dataGraphConfig.getGraph().getType());
	...
	private CloudGraphConfiguration unmarshalConfig(String configFileName,
			CloudGraphConfigDataBinding binding) {
		try {
			InputStream stream = CloudGraphConfiguration.class
					.getResourceAsStream(configFileName);
			if (stream == null)
				stream = CloudGraphConfig.class.getClassLoader()
						.getResourceAsStream(configFileName);
			if (stream == null)
				throw new CloudGraphConfigurationException(
	 ...
	 * 
	 * @see
	 * org.cloudgraph.config.TableMapping#getTable(javax.xml.namespace.QName)
	 */
	@Override
	public TableConfig getTable(QName typeName) {
		...
		if (result == null)
			throw new CloudGraphConfigurationException(
					"no HTable configured for " + " graph URI '"
							+ typeName.toString() + "'");
		return result;
	}
	...
	public TableConfig findTable(Type type) {
		return this.graphURIToTableMap.get(((PlasmaType) type)
				.getQualifiedName());
	}
	...
	private void collectTypeHierarchy(PlasmaType type,
			Map<QName, PlasmaType> map) {
	...
	public String getTableName(QName typeName) {
		...
		if (result == null)
			throw new CloudGraphConfigurationException(
					"no HTable configured for" + " CloudGraph '"
							+ typeName.toString() + "'");
	 ...
	 * 
	 * @see
	 * org.cloudgraph.config.TableMapping#findDataGraph(javax.xml.namespace.
	 * QName)
	 */
	@Override
	...
	public boolean uniqueChecks() {
		if (uniqueChecksVar == null) {
			uniqueChecksVar = getTablePropertyBoolean(
					ConfigurationProperty.CLOUDGRAPH___UNIQUE___CHECKS,
	...
	public boolean tombstoneRows() {
		if (tombstoneRowsVar == null) {
			tombstoneRowsVar = getTablePropertyBoolean(
					ConfigurationProperty.CLOUDGRAPH___TOMBSTONE___ROWS,
	...
	/**
	 * Checks for system property, then value changed at table level, then
	 * global config level.
	 * 
	 * @param prop
	 *            the config prop

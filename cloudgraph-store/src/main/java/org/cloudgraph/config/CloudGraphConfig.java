...
public class CloudGraphConfig implements Config {
  ...
  private CloudGraphConfig() {
    ...
    try {
      ...
      if (propertiesStream == null)
        propertiesStream = CloudGraphConfig.class.getClassLoader().getResourceAsStream(
    ...
    try {
      ...
      String fileName = EnvProperties.instance().getProperty(PROPERTY_NAME_CLOUDGRAPH_CONFIG);
  ...
  private void mapTable(TableConfig tableConfig) {
    if (this.tableNameToTableMap.get(tableConfig.getQualifiedName()) != null)
      throw new CloudGraphConfigurationException(
          "a table definition already exists for qualified name '" + tableConfig.getQualifiedName()
              + "'");
  ...
  public TableConfig getTable(QName typeName) {
    ...
    if (result == null)
      throw new CloudGraphConfigurationException("no HTable configured for " + " graph URI '"
          + typeName.toString() + "'");
    return result;
  }
  ...
  public TableConfig getTable(String tableNamespace, String tableName) {
    TableConfig result = this.tableNameToTableMap.get(tableName);
    if (result == null)
      throw new CloudGraphConfigurationException("no table configured for" + " name '"
          + tableName.toString() + "'");
    return result;
  }
  ...
  public String getTableName(QName typeName) {
    ...
    if (result == null)
      throw new CloudGraphConfigurationException("no HTable configured for" + " CloudGraph '"
          + typeName.toString() + "'");

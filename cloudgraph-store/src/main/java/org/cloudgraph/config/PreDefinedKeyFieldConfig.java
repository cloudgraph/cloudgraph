...
public class PreDefinedKeyFieldConfig extends KeyFieldConfig {
		...
		case URI: 
			...
			if (result == null || result.length() == 0) {
				if (log.isDebugEnabled())
				    log.debug("no physical URI name for type, "
				    		+ rootType.toString()
				    		+ ", defined - using URI");
			...
			DataObject dataObject) 
	{
		...
		case URI: 
			...
			if (result == null || result.length == 0) {
				if (log.isDebugEnabled())
				    log.debug("no physical URI name bytes for type, "
				    		+ rootType.toString()
	...
	public byte[] getKeyBytes(PlasmaType type, EntityMetaField metaField) {
		switch (this.getName()) {
		case URI: 
		case TYPE:
		case PKG:
	...
	public byte[] getKeyBytes(PlasmaType type, EdgeMetaField metaField) {
		switch (this.getName()) {
		case URI: 
		case TYPE:
		case PKG:
	...
	public byte[] getKeyBytes(EntityMetaField metaField) 
	{
		byte[] result = null;
		switch (this.getName()) {
		case PROPERTY:
			result = metaField.name().getBytes(charset);
			break;
		default:
		    throw new CloudGraphConfigurationException("invalid predefined key field name, "
		    		+ this.getName().name() + " - cannot get this predefined field from a metaField");
		}
		
		return result;
	}

	public byte[] getKeyBytes(EdgeMetaField metaField) 
	{
		byte[] result = null;
		switch (this.getName()) {
		case PROPERTY:

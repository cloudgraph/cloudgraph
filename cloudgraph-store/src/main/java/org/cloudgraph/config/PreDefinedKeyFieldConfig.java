...
public class PreDefinedKeyFieldConfig extends KeyFieldConfig {
	...
	private static final Log log = LogFactory
			.getLog(PreDefinedKeyFieldConfig.class);
	private PredefinedField field;

	public PreDefinedKeyFieldConfig(PredefinedField field, int seqNum,
			int totalFields) {
		...
		switch (this.getName()) {
				...
				if (result == null || result.length() == 0) {
					if (log.isDebugEnabled())
						log.debug("no physical URI name for type, "
								+ rootType.toString() + ", defined - using URI");
			...
			default :
				throw new CloudGraphConfigurationException(
						"invalid row key field name, " + this.getName().name()
	...
	public byte[] getKeyBytes(PlasmaType type, PlasmaProperty property) {
		switch (this.getName()) {
			...
			default :
				throw new CloudGraphConfigurationException(
						"invalid predefined key field name, "
								+ this.getName().name()

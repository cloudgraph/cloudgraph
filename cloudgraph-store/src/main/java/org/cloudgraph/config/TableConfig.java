...
public class TableConfig {
	...
	public TableConfig(Table table, Config config) {
		super();
		this.table = table;
		this.config = config;
	...
	public TableConfig(Table table) {
		this(table, CloudGraphConfig.getInstance());
	}
	...
	public boolean tombstoneRows() {
		if (tombstoneRowsVar == null) {
			tombstoneRowsVar = getTablePropertyBoolean(
					...
					this.table.isTombstoneRows(), true);
		}
	...
	private String getTablePropertyString(ConfigurationProperty prop,
		...
		String value = this.config.getConfigProperties()
				.getString(prop.value());
		if (value != null) {
			result = value;
		} else {
			if (defaultValue != tableValue) { // default overridden, take the
				...
				if (tableProp != null) {
					...
					Property globalProp = this.config
							.findProperty(prop.value());
					if (globalProp != null) {

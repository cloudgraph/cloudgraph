...
/**
 * Stores sequence generator state information for a graph row using JAXB
 * data binding and validation.  
 * <p>
 * Every property of every entity regardless of whether it is part of a collection must 
 * contain a sequence number that is unique for the entity type within the local graph 
 * row. This is essential as a single graph may contain multiple collections of the same 
 * entity type. The current sequence number for each entity type is maintained as state 
 * information for the graph row. This is minimal and consists of a simple mapping of 
 * type physical names, mapped to long integers. At runtime this state information is 
 * used within a sequence generator, with changes being written back to the graph 
 * row on commit.   
 * </p>
 * 
...
public class BindingSequenceGenerator implements SequenceGenerator {
    ...
    public BindingSequenceGenerator(java.util.UUID rootUUID, StateMarshalingContext context) {    	
		...
		for (URI uri : model.getURIS()) {
			...
			for (TypeEntry type : uri.getTypes()) {
				...
				if (this.typeNameMap.containsKey(typeHash))
					throw new StateException("mulitple types mapped to uri/type '"
				        +uri.getName() + "/" + type.getName()+"'");
	...
	private int getTypeHashCode(String uriName, 
			String typeName) {
		int result = 1;
	...
	public Long nextSequence(PlasmaType type) {
		...
		if (uriName == null) {
			log.warn("no URI physical name available for type, " + type
		...
		if (typeName == null) {
			log.warn("no physical name available for type, " + type

 ...
 */
package org.cloudgraph.rocksdb.test;
 ...
 * @since 0.5
 */
public abstract class SocialGraphModelTest extends RocksDBTestCase {
  ...
  protected GraphInfo createActor() throws IOException {
    GraphInfo info = new GraphInfo();
  ...
  protected QActor createActorBlogGraphQuery(String name, String id) {
    ...
    root.where(root.name().eq(name).and(root.id().eq(id)));
  ...
  protected QActor createActorGraphTableScanQuery() {
    QActor root = QActor.newQuery();
    root.select(root.wildcard()).select(root.sourceEdge().wildcard())
        .select(root.sourceEdge().source().wildcard())
        .select(root.sourceEdge().target().wildcard()).select(root.targetEdge().wildcard())

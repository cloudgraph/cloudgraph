...
public class Table implements TableName {
  ...
  public Table(TableName tableName, RocksDB client, TableMapping tableMapping,
  ...
  public void writeIncrements(List<Row> rows, Object[] results) {
    ...
    try {
      for (Row row : rows) {
        RowMutation mutation = RowMutation.class.cast(row); // FIXME:
        // cast
  ...
  public Result get(Get get) {
    if (get.getColumnFilter() == null)
      throw new IllegalStateException("expected column filter");
  ...
  public Result[] get(List<Get> gets) {
    ...
    org.cloudgraph.rocksdb.filter.Filter columnFilter = gets.get(0).getColumnFilter();
    List<byte[]> values;
    try {
    ...
    for (byte[] value : values) {
      ...
      KeyInfo ki = new KeyInfo(key, this.tableMapping.getDataColumnFamilyName());
      results[i] = new Result(ki, columns);
      i++;
    }
    return results;
  }
  ...
  public Result[] scan(Scan scan) {
    ...
    try {
      ...
      for (iterator.seekToFirst(); iterator.isValid(); iterator.next()) {
        ...
        Column[] columns = TupleCodec.decodeRow(value);
        columns = this.filter(columns, scan.getColumnFilter());
        if (log.isDebugEnabled())
          log.debug("scan: " + this.toDebugString(key, columns, true));
        Result result = new Result(ki, columns);
        list.add(result);
        prevIterKey = iterKey;
      }
    } catch (RocksDBException e) {
  ...
  private Column[] filter(Column[] columns, Filter columnFilter) {
    List<Column> filtered = new ArrayList<>();
    if (!FilterList.class.isInstance(columnFilter)) {
      if (MultiColumnPrefixFilter.class.isInstance(columnFilter)) {
        MultiColumnPrefixFilter prefixFilter = MultiColumnPrefixFilter.class.cast(columnFilter);
      ...
      } else if (MultiColumnCompareFilter.class.isInstance(columnFilter)) {
        MultiColumnCompareFilter perfixFilter = MultiColumnCompareFilter.class.cast(columnFilter);
        ...
        throw new IllegalStateException("unknown filter class, " + columnFilter.getClass());
      }
    } else {
      throw new IllegalStateException("unknown filter class, " + columnFilter.getClass());
    }
  ...
  private String toDebugString(Column column, boolean values) {
    ...
    if (values) {
      buf.append("='");
      String value = Bytes.toString(column.getData());
      value = value.replace("  ", "");
      value = value.replace('\n', ' ');
      if (value.length() > 60) {

...
import org.cloudgraph.rocksdb.filter.InitialFetchColumnFilterAssembler;
import org.cloudgraph.rocksdb.filter.PredicateRowFilterAssembler;
...
import org.cloudgraph.rocksdb.results.NoOpResultsComparator;
import org.cloudgraph.rocksdb.results.ParallelSlidingResultsAssembler;
import org.cloudgraph.rocksdb.results.ResultsAggregator;
...
import org.cloudgraph.rocksdb.results.SlidingResultsAssembler;
 ...
 * {@link org.cloudgraph.rocksdb.graph.HBaseGraphAssembler assembler} is used to
...
public class GraphQuery implements QueryDispatcher {
  ...
  protected PlasmaDataGraph[] execute(Query query, PlasmaType type, SelectionCollector selection,
    ...
    try {
      ...
      if (partialScans.size() > 0 || completeKeys.size() > 0) {
        ...
        Scan fullScan = new Scan(columnFilter);
        this.execute(fullScan, rootTableReader, resultsCollector);
      }
   ...
   */
  protected RocksDBFilterAssembler createRootColumnFilterAssembler(PlasmaType type,
      SelectionCollector collector, ServiceContext context) {
    RocksDBFilterAssembler columnFilterAssembler = null;

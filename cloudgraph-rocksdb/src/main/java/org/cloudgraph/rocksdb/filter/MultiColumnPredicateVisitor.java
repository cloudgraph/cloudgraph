...
public class MultiColumnPredicateVisitor extends PredicateVisitor {
  ...
  // protected FilterList qualifierFilterList = new
  // FilterList(FilterList.Operator.MUST_PASS_ONE);
  // protected FilterList valueFilterList = new
  ...
  public void start(Literal literal) {
    ...
    // Object valueObj =
    // DataConverter.INSTANCE.fromString(this.contextProperty.getType(),
    // content);
    // byte[] valueBytes =
    // AerospikeDataConverter.INSTANCE.toBytes(this.contextProperty, valueObj);
    // valueFilter = new ValueFilter(this.contextHBaseCompareOp, new
    // BinaryComparator(valueBytes));
    // } else {
    // if (!validateWildcardDataFlavor(this.contextProperty.getDataFlavor()))
    // throw new
    // InvalidOperatorException(this.contextWildcardOperator.getValue().name(),
  ...
  public void start(LogicalOperator operator) {
    ...
    case OR:
      // FilterList top = this.filterStack.peek();
      // if (top.getOperator().ordinal() !=
      // FilterList.Operator.MUST_PASS_ONE.ordinal()) {

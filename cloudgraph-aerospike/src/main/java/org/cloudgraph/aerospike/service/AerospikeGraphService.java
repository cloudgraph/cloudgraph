...
public class AerospikeGraphService implements PlasmaDataAccessService {

  private static Log log = LogFactory.getLog(AerospikeGraphService.class);
  protected ServiceContext context;

  public AerospikeGraphService() {
  }
  ...
  public Observable<DataGraph> findAsStream(Query query) {
     ...
     * IllegalArgumentException("expected non-null 'query' argument"); //
     * validate(query); if (log.isDebugEnabled()) { log(query); }
     * GraphStreamQuery dispatcher = null; try { dispatcher = new
     * GraphStreamQuery(this.context); Timestamp snapshotDate = new
     * Timestamp((new Date()).getTime()); return dispatcher.findAsStream(query,
     * snapshotDate); } finally { if (dispatcher != null) dispatcher.close(); }
     */
  }
  ...
  public Observable<DataGraph> findAsStream(Query query, int maxResults) {
     ...
     * IllegalArgumentException("expected non-null 'query' argument");
     * validate(query); if (log.isDebugEnabled()) { log(query); }
     * GraphStreamQuery graphQuery = null; try { graphQuery = new
     * GraphStreamQuery(this.context); return graphQuery.findAsStream(query, -1,
     * new Timestamp((new Date()).getTime())); } finally { if (graphQuery !=
     * null) graphQuery.close(); }
     */
  }

...
/**
 * Aerospike specific data converter, delegates to {@link DataConverter} under
 * certain conditions.
 * 
 * @author Scott Cinnamond
 * @since 0.5
 */
public class AerospikeDataConverter implements org.cloudgraph.store.DataConverter {
  private static Log log = LogFactory.getFactory().getInstance(AerospikeDataConverter.class);

  static public volatile AerospikeDataConverter INSTANCE = initializeInstance();

  private AerospikeDataConverter() {
  }
   ...
   */
  private static synchronized AerospikeDataConverter initializeInstance() {
    if (INSTANCE == null)
      INSTANCE = new AerospikeDataConverter();
    return INSTANCE;
  }

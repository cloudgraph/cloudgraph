...
import org.cloudgraph.aerospike.connect.AerospikeConnectionManager;
...
import org.cloudgraph.aerospike.filter.InitialFetchColumnFilterAssembler;
import org.cloudgraph.aerospike.filter.PredicateRowFilterAssembler;
...
import org.cloudgraph.aerospike.results.NoOpResultsComparator;
import org.cloudgraph.aerospike.results.ParallelSlidingResultsAssembler;
import org.cloudgraph.aerospike.results.ResultsAggregator;
import org.cloudgraph.aerospike.results.ResultsAssembler;
import org.cloudgraph.aerospike.results.ResultsComparator;
import org.cloudgraph.aerospike.results.ResultsComparatorAssembler;
import org.cloudgraph.aerospike.results.SlidingResultsAssembler;
import org.cloudgraph.aerospike.scan.CompleteRowKey;
//import org.cloudgraph.aerospike.scan.FuzzyRowKey;
import org.cloudgraph.aerospike.scan.PartialRowKey;
import org.cloudgraph.aerospike.scan.PartialRowKeyScanAssembler;
import org.cloudgraph.aerospike.scan.ScanCollector;
 ...
 * {@link org.cloudgraph.aerospike.graph.HBaseGraphAssembler assembler} is used
 * to reconstruct and return the original graph structure from the resulting
 * HBase row.
 * </p>
 * <p>
...
public class GraphQuery implements QueryDispatcher {
  ...
  private PlasmaDataGraph[] find(Query query, PlasmaType type, Timestamp snapshotDate) {
    ...
    // in the Select.
    AerospikeFilterAssembler columnFilterAssembler = createRootColumnFilterAssembler(type,
    ...
    return execute(query, type, selection, columnFilter, whereSyntaxTree, orderingComparator,
        groupingComparator, havingSyntaxTree, partialScans, completeKeys, snapshotDate);
  }
  ...
  protected PlasmaDataGraph[] execute(Query query, PlasmaType type, SelectionCollector selection,
    ...
    // execute
    Connection connection = AerospikeConnectionManager.instance().getConnection();
  ...
  protected Scan createScan(PartialRowKey partialRowKey, Filter columnFilter) {
    Scan scan = null;
    if (partialRowKey.getScanLiterals() != null) {
      scan = new Scan(partialRowKey.getScanLiterals(), columnFilter);
    } else {
      scan = new Scan(columnFilter);
    }
   ...
   */
  protected AerospikeFilterAssembler createRootColumnFilterAssembler(PlasmaType type,
      SelectionCollector collector, ServiceContext context) {
    AerospikeFilterAssembler columnFilterAssembler = null;

 ...
 * @since 2.0.0
 */
public class ColumnPredicateVisitor extends PredicateVisitor implements AerospikeConstants {
  ...
  public void start(Property property) {
    ...
    ColumnInfo ci = new ColumnInfo(colKeyStr, colKey, this.contextType, this.contextProperty);
    this.columnMap.put(colKeyStr, ci);
    if (log.isDebugEnabled())
      log.debug("collected " + ci);

    for (int i = 1; i < FETCH_HACK; i++) {
      byte[] edgecolKey = this.columnKeyFac.createColumnKey(this.contextType, i,
          this.contextProperty);
      String edgeColumnKeyStr = Bytes.toString(edgecolKey);
      ci = new ColumnInfo(edgeColumnKeyStr, edgecolKey, this.contextType, this.contextProperty);
      this.columnMap.put(edgeColumnKeyStr, ci);
      if (log.isDebugEnabled())
        log.debug("collected " + ci);
    }
    ...
    if (!this.contextProperty.getType().isDataType()) {
      for (EntityMetaKey metaField : EntityMetaKey.values()) {
        ...
        ci = new ColumnInfo(colKeyStr, colKey, this.contextType, metaField.getStorageType());
        this.columnMap.put(colKeyStr, ci);
        if (log.isDebugEnabled())
          log.debug("collected: " + ci);
        ...
        // column keys
        for (int i = 1; i < FETCH_HACK; i++) {
          byte[] edgecolKey = this.columnKeyFac.createColumnKey(this.contextType, i, metaField);
          String edgeColumnKeyStr = Bytes.toString(edgecolKey);
          ci = new ColumnInfo(edgeColumnKeyStr, edgecolKey, this.contextType, DataType.Bytes);
          this.columnMap.put(edgeColumnKeyStr, ci);
          if (log.isDebugEnabled())
            log.debug("collected: " + ci);
          for (EdgeMetaKey edgeMetaKey : EdgeMetaKey.values()) {
            String edgeColMetaKey = edgeColumnKeyStr + "#" + edgeMetaKey.code();
            ci = new ColumnInfo(edgeColMetaKey, edgeColMetaKey.getBytes(this.charset),
                this.contextType, edgeMetaKey.getStorageType());
            this.columnMap.put(edgeColMetaKey, ci);
            if (log.isDebugEnabled())
              log.debug("collected: " + ci);
          }
        }

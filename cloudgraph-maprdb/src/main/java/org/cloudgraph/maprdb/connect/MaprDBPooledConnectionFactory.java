 ...
 * @since 0.6.3
 */
public class MaprDBPooledConnectionFactory extends BasePooledObjectFactory<Connection> {
  private static Log log = LogFactory.getLog(MaprDBPooledConnectionFactory.class);

  private Configuration config;
  private ObjectPool<Connection> pool;

  public MaprDBPooledConnectionFactory(Configuration config) {
    super();
    this.config = config;
  }
  ...
  public Connection create() throws Exception {
    ...
    return new MaprDBConnection(con, this.pool, this.config);
  }

...
public abstract class CommitComparator
		implements
			Comparator<CoreDataObject>,
			Serializable {
	...
	protected boolean isSingularRelation(DataObject target, DataObject source) {
		return (((PlasmaType) target.getType()).isRelation(
				(PlasmaType) source.getType(), AssociationPath.singular));
	}
	...
	protected boolean isLinked(DataObject other, Object value) {
		if (value instanceof List) {
			...
			for (DataObject dataObject : list) {
				...
				if (dataNode.getUUIDAsString().equals(
						((PlasmaNode) other).getUUIDAsString())) {
					return true;
				}
			}
		} else {

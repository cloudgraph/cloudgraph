...
/**
 * Comparator which imposes a commit ordering on created objects based on
 ...
 * are detected and the "parent" types are ordered first. Where the types are
 * the same, singular relations may still exist from/to the same type forming a
...
public class CreatedCommitComparator extends CommitComparator {
	...
	public int compare(CoreDataObject source, CoreDataObject target) {
		...
		PlasmaChangeSummary changeSummary = (PlasmaChangeSummary) source
		...
		if (targetType.getQualifiedName() != sourceType.getQualifiedName()) {
			if (log.isDebugEnabled())
				log.debug("comparing types: " + sourceType.toString() + " / "
						+ targetType.toString());

			if (isSingularRelation(source, target)) { // source is less than
														// target
				if (log.isDebugEnabled())
					log.debug("(return -1) - singular relation from source: "
							...
							+ targetType.toString());
				return -1;
			} else if (isSingularRelation(target, source)) { // source is
																// greater than
																// target
				if (log.isDebugEnabled())
			...
			if (log.isDebugEnabled())
				log.debug("comparing data objects: " + source.toString()
						+ " / " + target.toString());

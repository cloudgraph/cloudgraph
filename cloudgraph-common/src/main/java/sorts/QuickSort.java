...
/**
 * Quicksort algorithm Average Time Complexity: O(n log n) Worst Time
 * Complexity: O(n*n) Memory Complexity: O(log n) Stable: no
 */
...
public class QuickSort extends Sorter {
	...
	private <T extends Comparable<? super T>> void sort(T[] a, int s_low,
			int s_high) {
		int low = s_low;
		int high = s_high;
		if (low == high - 1) {

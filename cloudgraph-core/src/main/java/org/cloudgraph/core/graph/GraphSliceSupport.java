...
class GraphSliceSupport {
  ...
  public List<CellValues> filter(PlasmaType contextType, int level, EdgeReader edgeReader,
    ...
    DistributedGraphReader sliceGraphReader = new DistributedGraphReader(contextType,
        predicateSelection.getTypes(), existingReader.getConnection(), this.serviceContext);
    ...
    CoreGraphAssembler graphAssembler = new GraphAssembler(contextType, predicateSelection,
    ...
    // predicateSelection, contextType, this.serviceContext.getStoreMapping());
    // // use predicate
    // // selection not
    // // entire
    // // graph selection
    ...
    for (byte[] rowKey : graphEvalRowKeys) {
      ...
      // edge UUID
      Get get = this.serviceContext.getClientFactory().createGet(rowKey);
      // Get get = new Get(rowKey);
      get.setFilter(columnFilter);
      gets.add(get);
    }
    DataGraphMapping graphConfig = StoreMapping.getInstance().getDataGraph(
        contextType.getQualifiedName(), this.serviceContext.getStoreMapping());
    ...
    for (Result resultRow : rows) {
      ...
      graphAssembler.assemble(this.serviceContext.getClientFactory().createCellValues(resultRow));
      ...
      CellValues values = this.serviceContext.getClientFactory().createCellValues(
          resultRow.getRow());
  ...
  public Set<Long> fetchSequences(PlasmaType contextType, Where where, RowReader rowReader,
    ...
    DataGraphMapping graphConfig = StoreMapping.getInstance().getDataGraph(
        rootType.getQualifiedName(), this.serviceContext.getStoreMapping());
    Get get = this.serviceContext.getClientFactory().createGet(rowReader.getRowKey());
  ...
  public Result fetchResult(Get get, TableOperation tableOperation, DataGraphMapping graphConfig)
    ...
    if (log.isDebugEnabled())
      try {
        log.debug("get filter: " + get.getFilter().printFilterTree());
  ...
  public Result[] fetchResult(List<Get> gets, TableOperation tableOperation,
    ...
    if (log.isDebugEnabled())
      try {
        log.debug("get filter: " + gets.get(0).getFilter().printFilterTree());
      } catch (IOException e1) {
      }
  ...
  public Map<Long, Long> fetchSequences(Scan scan, TableReader tableReader,
    ...
    if (log.isDebugEnabled())
      try {
        log.debug("scan filter: " + scan.getFilter().printFilterTree());
      } catch (IOException e1) {
      }
  ...
  public void loadBySequenceList(Set<Long> sequences, Set<Property> properties,
    ...
    if (anyReferenceProperties(properties)) {
      ColumnFilterFactory filterFac = this.serviceContext.getColumnFilterFactory();
      Filter filter = filterFac.createStatefullBinaryPrefixColumnFilter(rootType, properties,
          sequences, contextType, edgeReader);
      get.setFilter(filter);
    } else {
    ...
    if (log.isDebugEnabled())
      try {
        if (get.getFilter() != null) {
          log.debug("executing get: " + get.getFilter().printFilterTree());
        } else {
    ...
    if (log.isDebugEnabled())
      try {
        if (scan.getFilter() != null)
          log.debug("executing scan: " + scan.getFilter().printFilterTree());
        else
          log.debug("executing scan... ");
      } catch (IOException e1) {
      }

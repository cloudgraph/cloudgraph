 ...
 * @since 0.5
 */
public abstract class CellValues {

  protected byte[] rowKey;
  protected Map<Integer, Map<Integer, byte[]>> familyMap;

  /**
   * Whether the complete graph selection for context type is found within the
   * cell values
   */
  protected boolean completeSelection = false;
  ...
  public void addColumn(KeyValue kv) {
    this.addColumn(kv.getFamily(), kv.getQualifier(), kv.getValue());
  }

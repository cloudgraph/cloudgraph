 ...
 * @since 0.5
 */
public class PartialRowKeyAssembler implements RowKeyAssembler, PartialRowKey {
  private static Log log = LogFactory.getLog(PartialRowKeyAssembler.class);
  ...
  private PartialRowKeyAssembler() {
  }
    ...
    for (int i = 0; i < fieldCount; i++) {
      ...
      if (dataField instanceof MetaKeyFieldMapping) {
        ...
        for (ScanLiteral scanLiteral : scanLiterals) {
          ...
          // if not last field
          if (i < (fieldCount - 1)
              && scanLiteral.getRelationalOperator().ordinal() == RelationalOperatorName.EQUALS
                  .ordinal()) {

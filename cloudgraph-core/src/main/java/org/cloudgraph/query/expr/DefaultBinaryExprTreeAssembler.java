 ...
 * 
 */
public abstract class DefaultBinaryExprTreeAssembler
		extends
			ExpresionVisitorSupport implements ExprAssembler {
	private static Log log = LogFactory
			.getLog(DefaultBinaryExprTreeAssembler.class);
	 ...
	 * {@link org.plasma.query.model.Expression expression} consuming each term
	 * in the {@link org.plasma.query.model.Expression expression}.
	 * 
	...
	/**
	 * Processes operators and operands currently staged returning an
	...
	private Expr assemble() {
		...
		if (this.operands.peek() instanceof Literal) {
			...
			throw new IllegalStateException("unknown opearand, "
					+ this.operands.peek());
	...
	private void consume(Term term) {
		...
		// assemble a node based on operator precedence
				...
				|| term.getLogicalOperator() != null
				|| term.getRelationalOperator() != null
				|| term.getWildcardOperator() != null) {
			Operator oper = null;
			if (term.getGroupOperator() != null)
				...
				oper = new Operator(term.getLogicalOperator(),
						this.precedenceMap);
			else if (term.getWildcardOperator() != null)
				oper = new Operator(term.getWildcardOperator(),
						this.precedenceMap);
			else
				oper = new Operator(term.getRelationalOperator(),
						this.precedenceMap);
	...
	private boolean isGroupPair(Operator right, Operator left) {
		if (right.getOperator() instanceof GroupOperator) {
			...
			if (groupRight.getValue().ordinal() == GroupOperatorValues.RP_1
					.ordinal()) {
				if (left.getOperator() instanceof GroupOperator) {
					GroupOperator groupLeft = (GroupOperator) left
							.getOperator();
					if (groupLeft.getValue().ordinal() == GroupOperatorValues.LP_1
							.ordinal()) {
						return true;
					}
				}

...
public abstract class ExpresionVisitorSupport extends DefaultQueryVisitor {
	...
	/**
	 * Returns a count of logical operators which match the given logical
	 * operator found within the child expressions for the given expression.
	 * 
	 * @param expression
	 *            the expression
	 * @return a count of logical operators which match the given logical
	 *         operator found within the child expressions for the given
	 *         expression.
	 */
	protected int getLogicalOperatorCount(Expression expression,
			LogicalOperatorValues operator) {
		int result = 0;
		for (Term term : expression.getTerms())
			if (term.getLogicalOperator() != null) {
				if (term.getLogicalOperator().getValue().ordinal() == operator
						.ordinal())
					result++;
			}

...
public class GraphRecognizerSupport {
	 ...
	 */
	public void collect(DataObject targetObject, Property property, Path path,
			int pathIndex, List<Object> values) {
		PlasmaType targetType = (PlasmaType) targetObject.getType();
		if (path != null && pathIndex < path.getPathNodes().size()) {
			AbstractPathElement pathElem = path.getPathNodes().get(pathIndex)
					.getPathElement();
			...
			if (!endpointProp.getType().isDataType())
				throw new GraphServiceException(
						"expected datatype property for, " + endpointProp);
		...
		switch (property.getDataFlavor()) {
			...
			case real :
				if (Number.class.isAssignableFrom(propertyValue.getClass())) {
					...
					result = evaluate(propertyNumberValue, operator,
							literalNumberValue);
					...
					Boolean literalBooleanValue = (Boolean) this.dataConverter
							.convert(property.getType(), literal);
					result = evaluate(propertyBooleanValue, operator,
							literalBooleanValue);
				} else
			...
			case string :
				...
				String literalStringValue = (String) this.dataConverter
						.convert(property.getType(), literal);
				result = evaluate(propertyStringValue, operator,
						literalStringValue);
				break;
			case temporal :
				switch (dataType) {
					case Date :
						...
						Date literalDateValue = (Date) this.dataConverter
								.convert(property.getType(), literal);
						result = evaluate(propertyDateValue, operator,
								literalDateValue);
						break;
					default :
						propertyStringValue = (String) propertyValue;
						literalStringValue = (String) this.dataConverter
								.convert(property.getType(), literal);
						result = evaluate(propertyStringValue, operator,
								literalStringValue);
						break;
	...
	private boolean evaluate(Date propertyValue,
			RelationalOperatorValues operator, Date literalValue) {
	...
	private boolean evaluate(Number propertyValue,
			RelationalOperatorValues operator, Number literalValue) {
	...
	private boolean evaluate(Boolean propertyValue,
			RelationalOperatorValues operator, Boolean literalValue) {
	...
	private boolean evaluate(String propertyValue,
			RelationalOperatorValues operator, String literalValue) {
		try {
			int comp = propertyValue.compareTo(literalValue);
			return evaluate(operator, comp);
	...
	private boolean evaluate(String propertyValue,
			WildcardOperatorValues operator, String literalValue) {
	...
	@SuppressWarnings("rawtypes")
	class NumberComparator<T extends Number & Comparable>
			implements
				Comparator<T> {

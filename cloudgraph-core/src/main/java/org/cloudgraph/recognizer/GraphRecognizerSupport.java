...
public class GraphRecognizerSupport {
			...
			String literal)
	{
		...
		case real:
			if (Number.class.isAssignableFrom(propertyValue.getClass())) {
				...
				Boolean literalBooleanValue = (Boolean)this.dataConverter.convert(property.getType(), literal);
				result = evaluate(propertyBooleanValue, 
					operator,
					literalBooleanValue); 
			}
			else
				throw new GraphServiceException("unexpected instanceof " 
					...
					+ property+ " with data flavor " + property.getDataFlavor());
		    break;
		case string:
			...
			Boolean literalValue)
	{
		...
		int comp = this.booleanComparator.compare(propertyValue, 
				literalValue);
		return evaluate(operator, comp);
	}
